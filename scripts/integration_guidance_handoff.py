...
NATIVE_OPERATIONS = {"pc_handoff_current": "handoff_current_work"}
...
class HandoffFixture:
    ...
    def respond(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if operation != "handoff_current_work" or self.prepared is not None:
            raise ValueError("Unexpected operation after or during a temporary Handoff: " + name)
        request = models.HandoffCurrentWorkRequest.model_validate({"scope_id": "fixture-scope", **arguments})
        payload = request.model_dump(mode="json", by_alias=True)
        if payload["scope_id"] != "fixture-scope":
            message = "Handoff operation changed the bound Scope"
            raise ValueError(message)
        result = self._respond(payload)
    ...
    def _respond(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        source = {"name": "content", "source_id": payload["source_id"]}
