...
class MemoryService:
        ...
        try:
            ...
            for result_item in all_results:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to fetch full memory info for {memory_id}: {e}, using result_item data"
                    )
                ...
                if (
                    "created_at" in result_item
                    and result_item["created_at"] is not None
                ):
                    normalized_memory["created_at"] = result_item["created_at"]
                if (
                    "updated_at" in result_item
                    and result_item["updated_at"] is not None
                ):
        ...
        try:
            ...
            if not memory.get("content") or memory.get("content") == "":
                try:
                    ...
                    logger.warning(
                        f"Failed to get content from storage payload for memory {memory_id}: {e}"
                    )
        ...
        try:
            ...
            for item in memories_list:
                if isinstance(item, dict):
                    ...
                    logger.warning(
                        f"Skipping non-dict item in memories list: {type(item)} - {item}"
                    )
    ...
    def update_memory(
        ...
        try:
            ...
            final_content = (
                content if content is not None else existing.get("content", "")
            )
    ...
    def batch_create_memories(
        ...
        for idx, memory_item in enumerate(memories):
            try:
                ...
                logger.error(
                    f"Failed to create memory at index {idx}: {e}", exc_info=True
                )
                failed.append(
                    {
                        "index": idx,
    ...
    def batch_update_memories(
        ...
        for idx, update_item in enumerate(updates):
            try:
                ...
                logger.error(
                    f"Failed to update memory at index {idx}: {e}", exc_info=True
                )
                failed.append(
                    {
                        "index": idx,
