    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.error(
                f"Failed to connect to SQLite database at {database_path}: {e}"
            )
            raise
    ...
    def search(
        ...
        if filters:
            ...
            if conditions:
                ...
                logger.info(
                    f"SQLite search with filters: {query_sql}, params: {query_params}"
                )
            else:
                logger.debug(
                    "SQLite search: filters provided but empty after processing"
                )
        else:
        ...
        with self._lock:
            ...
            for row in cursor.fetchall():
                ...
                results.append(
                    OutputData(id=vector_id, score=similarity, payload=payload)
                )
