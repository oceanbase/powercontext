...
from powermem.utils.utils import generate_snowflake_id, serialize_datetime
...
class OceanBaseVectorStore(VectorStoreBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        final_connection_args = {
            "host": host
            or connection_args.get(
                "host", constants.DEFAULT_OCEANBASE_CONNECTION["host"]
            ),
            "port": port
            or connection_args.get(
                "port", constants.DEFAULT_OCEANBASE_CONNECTION["port"]
            ),
            "user": user
            or connection_args.get(
                "user", constants.DEFAULT_OCEANBASE_CONNECTION["user"]
            ),
            "password": password
            or connection_args.get(
                "password", constants.DEFAULT_OCEANBASE_CONNECTION["password"]
            ),
            "db_name": db_name
            or connection_args.get(
                "db_name", constants.DEFAULT_OCEANBASE_CONNECTION["db_name"]
            ),
        }
        ...
        self.embedding_model_dims = (
            int(embedding_model_dims) if embedding_model_dims is not None else None
        )
    ...
    def _configure_vector_index_settings(self):
        ...
        try:
            ...
            if OceanBaseUtil.is_seekdb(self.obvector):
                logger.info(
                    "seekdb is adaptive mode, no need to configure sparse vector"
                )
                return
            ...
            if version_dict is None:
                logger.warning(
                    "Could not determine database version, please check the database version"
                )
                return
            ...
            if major >= 4 and minor >= 4 and patch >= 1:
                logger.info(
                    "OceanBase version >= 4.4.1, no need to configure sparse vector"
                )
                return
            ...
            with self.obvector.engine.connect() as conn:
                ...
                if row:
                    logger.info(
                        "ob_vector_memory_limit_percentage already set, skipping configuration"
                    )
                    return
                else:
                    conn.execute(
                    ...
                    conn.commit()
                    logger.info(
                        "Set ob_vector_memory_limit_percentage = 30 successfully"
                    )
    ...
    def _create_table_with_index_by_embedding_model_dims(self) -> None:
        ...
        cols = [
            ...
            Column(
                self.primary_field, BigInteger, primary_key=True, autoincrement=False
            ),
            # Vector field
        ...
        vidx_params.add_index(
            ...
            index_type=constants.OCEANBASE_SUPPORTED_VECTOR_INDEX_TYPES[
                self.index_type
            ],
        ...
        if self.hybrid_search:
            ...
            logger.debug(
                f"Added fulltext index configuration with parser '{self.fulltext_parser}' for table '{self.collection_name}'"
            )
        ...
        if self.include_sparse:
            if OceanBaseUtil.check_sparse_vector_version_support(self.obvector):
                ...
                logger.info(
                    f"Including sparse_embedding column in new table '{self.collection_name}'"
                )
                vidx_params.add_index(
                ...
                logger.debug(
                    f"Added sparse vector index configuration for table '{self.collection_name}'"
                )
            else:
                logger.warning(
    ...
    def create_col(
        self, name: str, vector_size: Optional[int] = None, distance: str = "l2"
    ):
        """Create a new collection."""
        try:
            if vector_size is None:
                raise ValueError(
                    "vector_size must be specified to create a collection."
                )
            distance = distance.lower()
            if distance not in ("l2", "inner_product", "cosine"):
                raise ValueError(
                    "distance must be one of 'l2', 'inner_product', or 'cosine'."
                )
            ...
            self._create_col()
            logger.info(
                f"Successfully created collection '{name}' with vector size {vector_size} and distance '{distance}'"
            )
    ...
    def _create_col(self):
        ...
        if not self.obvector.check_table_exists(self.collection_name):
            ...
            logger.info(
                f"Table {self.collection_name} already exists, preserving existing data"
            )
        ...
        if self.include_sparse:
            if not OceanBaseUtil.check_sparse_vector_ready(
    ...
    def insert(
        ...
        try:
            ...
            logger.debug(
                f"Successfully inserted {len(vectors)} vectors, generated Snowflake IDs: {generated_ids}"
            )
            return generated_ids

        except Exception as e:
            logger.error(
                f"Failed to insert vectors into collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise

    def _generate_where_clause(
        self, filters: Optional[Dict] = None, table=None
    ) -> Optional[List]:
        """
                    ...
                    match op:
                        ...
                        case "in":
                            if not isinstance(op_value, list):
                                raise TypeError(
                                    f"Value for $in must be a list, got {type(op_value)}"
                                )
                            conditions.append(column.in_(op_value))
                        case "nin":
                            if not isinstance(op_value, list):
                                raise TypeError(
                                    f"Value for $nin must be a list, got {type(op_value)}"
                                )
                            conditions.append(~column.in_(op_value))
                        case "like":
    ...
    def _get_standard_column_names(
        self, include_vector_field: bool = False
    ) -> List[str]:
        """
    ...
    def _parse_row_to_dict(
        self, row, include_vector: bool = False, extract_score: bool = True
    ) -> Dict:
        """
        ...
        if include_vector:
            ...
            if (
                self.include_sparse
                and hasattr(record, "sparse_embedding")
                and record.sparse_embedding is not None
            ):
    ...
    def _create_output_data(
        self, vector_id: int, text_content: str, score: float, metadata: Dict
    ) -> OutputData:
        """Create an OutputData object with standard structure."""
        return OutputData(
            id=vector_id, score=score, payload={"data": text_content, **metadata}
        )

    def _build_record_for_insert(
        self, vector: List[float], payload: Dict
    ) -> Dict[str, Any]:
        """
        ...
        if self.include_sparse and "sparse_embedding" in payload:
            ...
            if isinstance(sparse_embedding, dict):
                ...
                raise ValueError(
                    f"Sparse embedding must be a dict, got {type(sparse_embedding)}"
                )
    ...
    def _vector_search(
        ...
        try:
            ...
            results = self.obvector.ann_search(
                ...
                else OceanBaseUtil.normalize(query_vector),
            ...
            for row in results.fetchall():
                parsed = self._parse_row_to_dict(
                    row, include_vector=False, extract_score=True
                )
                ...
                if distance is None:
                    logger.warning(
                        f"Distance is None for vector_id {vector_id}, using default similarity 0.0"
                    )
                    similarity = 0.0
                ...
                search_results.append(
                    self._create_output_data(
                        vector_id, text_content, similarity, metadata
                    )
                )
            ...
            logger.error(
                f"Vector search failed in collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def _fulltext_search(
        ...
        fts_condition = text(
            f"MATCH({self.fulltext_field}) AGAINST(:query IN NATURAL LANGUAGE MODE)"
        ).bindparams(bindparam("query", query))
        ...
        try:
            ...
            columns = self._get_standard_select_columns() + [
                ...
                text(
                    f"MATCH({self.fulltext_field}) AGAINST(:query IN NATURAL LANGUAGE MODE) as score"
                ).bindparams(bindparam("query", query))
            ]
            ...
            try:
                ...
                like_query = f"%{query}%"
                like_condition = text(
                    f"{self.fulltext_field} LIKE :like_query"
                ).bindparams(bindparam("like_query", like_query))
                ...
                with self.obvector.engine.connect() as conn:
                    with conn.begin():
                        logger.info(
                            f"Executing LIKE fallback query with parameters: like_query={like_query}"
                        )
                        # Execute with parameter dictionary - the standard SQLAlchemy way
                        results = conn.execute(stmt)
                        rows = results.fetchall()
                ...
                logger.error(
                    f"Both full-text search and LIKE fallback failed: {fallback_error}"
                )
                return []
        ...
        for row in rows:
            ...
            fts_results.append(
                self._create_output_data(
                    parsed["vector_id"], parsed["text_content"], fts_score, metadata
                )
            )

        logger.info(
            f"_fulltext_search results, len : {len(fts_results)}, fts_results : {fts_results}"
        )
        return fts_results
    ...
    def _sparse_search(
        ...
        try:
            ...
            columns = self._get_standard_select_columns() + [
                ...
                text(
                    f"negative_inner_product({self.sparse_vector_field}, '{sparse_vector_str}') as score"
                )
            ]
            ...
            with self.obvector.engine.connect() as conn:
                with conn.begin():
                    logger.debug(
                        f"Executing sparse vector search query with sparse_vector: {sparse_vector_str}"
                    )
                    # Execute the query
                    results = conn.execute(stmt)
                    rows = results.fetchall()
    ...
    def _hybrid_search(
        ...
        with ThreadPoolExecutor(max_workers=3 if perform_sparse else 2) as executor:
            ...
            vector_future = executor.submit(
                self._vector_search, query, vectors, candidate_limit, filters
            )
            search_tasks.append(("vector", vector_future))

            # Submit full-text search
            fts_future = executor.submit(
                self._fulltext_search, query, candidate_limit, filters
            )
            ...
            if perform_sparse:
                sparse_future = executor.submit(
                    self._sparse_search, sparse_embedding, candidate_limit, filters
                )
        ...
        logger.debug(
            f"Coarse ranking completed, candidates: {len(coarse_ranked_results)}"
        )
    ...
    def _apply_rerank(
        self, query: str, candidates: List[OutputData], limit: int
    ) -> List[OutputData]:
        """
    ...
    def _calculate_quality_score(
        ...
        quality_score = sum(
            (weight / total_weight) * score for weight, score in active_paths
        )
    ...
    def _combine_search_results(
        ...
        if fusion_method == "rrf":
            return self._rrf_fusion(
                vector_results, fts_results, sparse_results, limit, k, sparse_embedding
            )
        else:
            return self._weighted_fusion(
                vector_results, fts_results, sparse_results, limit
            )
    ...
    def _weighted_fusion(
        ...
        vector_w = (
            self.vector_weight if self.vector_weight is not None else vector_weight
        )
        fts_w = self.fts_weight if self.fts_weight is not None else text_weight
        sparse_w = 0.0
        if self.include_sparse and sparse_results:
            sparse_w = (
                self.sparse_weight if self.sparse_weight is not None else sparse_weight
            )
        ...
        for doc_id, doc_data in combined_results.items():
            combined_score = (
                vector_w * doc_data["vector_score"]
    ...
    def delete(self, vector_id: int):
        ...
        try:
            ...
            logger.debug(
                f"Successfully deleted vector with ID: {vector_id} from collection '{self.collection_name}'"
            )
        except Exception as e:
            logger.error(
                f"Failed to delete vector with ID {vector_id} from collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def update(
        ...
        try:
            ...
            has_sparse_column = OceanBaseUtil.check_column_exists(
                self.obvector, self.collection_name, self.sparse_vector_field
            )
            ...
            if not existing_rows:
                logger.warning(
                    f"Vector with ID {vector_id} not found in collection '{self.collection_name}'"
                )
                return
            ...
            existing_sparse_embedding = (
                existing_rows[0][1]
                if has_sparse_column and len(existing_rows[0]) > 1
                else None
            )
            ...
            if payload is not None:
                ...
                for key, value in temp_record.items():
                    if key != self.primary_field and (
                        vector is not None or key != self.vector_field
                    ):
            ...
            if has_sparse_column and self.sparse_vector_field not in update_data:
                if existing_sparse_embedding is not None:
                    ...
                    logger.debug(
                        f"Preserving existing sparse_embedding for ID {vector_id}"
                    )
            ...
            logger.debug(
                f"Successfully updated vector with ID: {vector_id} in collection '{self.collection_name}'"
            )

        except Exception as e:
            logger.error(
                f"Failed to update vector with ID {vector_id} in collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def get(self, vector_id: int):
        ...
        try:
            ...
            parsed = self._parse_row_to_dict(
                rows[0], include_vector=True, extract_score=False
            )

            logger.debug(
                f"Successfully retrieved vector with ID: {vector_id} from collection '{self.collection_name}'"
            )
            return self._create_output_data(
                parsed["vector_id"], parsed["text_content"], 0.0, parsed["metadata"]
            )

        except Exception as e:
            logger.error(
                f"Failed to get vector with ID {vector_id} from collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def delete_col(self):
        ...
        try:
            if self.obvector.check_table_exists(self.collection_name):
                ...
                logger.warning(
                    f"Collection '{self.collection_name}' does not exist, skipping deletion"
                )
        except Exception as e:
            logger.error(
            ...
            for col in columns:
                if col[0] == self.vector_field:
                    ...
                    if col_type.startswith("VECTOR(") and col_type.endswith(")"):
                        dim_str = col_type[
                            7:-1
                        ]  # Extract dimension from "VECTOR(1536)"
                        return int(dim_str)
            return None
        except Exception as e:
            logger.warning(
                f"Failed to get vector dimension for table {self.collection_name}: {e}"
            )
            return None
    ...
    def col_info(self):
        ...
        try:
            ...
            logger.debug(
                f"Successfully retrieved info for collection '{self.collection_name}'"
            )
            return {
            ...
            logger.error(
                f"Failed to get collection info for '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def list(self, filters: Optional[Dict] = None, limit: Optional[int] = None):
        ...
        try:
            ...
            for row in results.fetchall():
                parsed = self._parse_row_to_dict(
                    row, include_vector=True, extract_score=False
                )
            ...
            logger.debug(
                f"Successfully listed {len(memories)} memories from collection '{self.collection_name}'"
            )
            return [memories]

        except Exception as e:
            logger.error(
                f"Failed to list memories from collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def reset(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to reset collection '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
    ...
    def _create_fulltext_index(self):
        try:
            ...
            with self.obvector.engine.connect() as conn:
                ...
                logger.debug(
                    "DEBUG: Fulltext index created successfully for '%s'",
                    self.collection_name,
                )
