...
from ..intelligence.plugin import EbbinghausIntelligencePlugin, IntelligentMemoryPlugin
...
from ..storage.factory import GraphStoreFactory, VectorStoreFactory
...
def _auto_convert_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "database" in config or ("llm" in config and "embedding" in config):
        ...
        if "database" in config:
            ...
            converted["vector_store"] = {"provider": "oceanbase", "config": {}}
...
class Memory(MemoryBase):
    ...
    def __init__(
        ...
        if isinstance(config, MemoryConfig):
            ...
            try:
                ...
                logger.warning(
                    f"Could not parse config as MemoryConfig: {e}, using dict mode"
                )
        ...
        self.storage_type = storage_type or self._get_provider(
            "vector_store", "oceanbase"
        )
        self.llm_provider = llm_provider or self._get_provider("llm", "mock")
        self.embedding_provider = embedding_provider or self._get_provider(
            "embedder", "mock"
        )
        ...
        if self.memory_config and hasattr(self.memory_config, "reranker"):
            ...
            if rerank_obj.enabled:
                try:
                    ...
                    logger.warning(
                        f"Failed to initialize reranker from MemoryConfig: {e}"
                    )
                    reranker = None
        else:
            ...
            if rerank_config is not None and rerank_config.get("enabled", False):
                try:
                    ...
                    logger.warning(
                        f"Failed to initialize reranker from JSON config: {e}"
                    )
                    reranker = None
        ...
        if self.enable_graph:
            ...
            if graph_store_config:
                ...
                config_to_pass = (
                    self.memory_config if self.memory_config else self.config
                )
        ...
        self.embedding = EmbedderFactory.create(
            self.embedding_provider, embedder_config, vector_store_config
        )
        ...
        if self.storage_type.lower() == "oceanbase" and include_sparse:
            ...
            if (
                self.memory_config
                and hasattr(self.memory_config, "sparse_embedder")
                and self.memory_config.sparse_embedder
            ):
            ...
            if sparse_config_obj:
                try:
                    ...
                    if hasattr(sparse_config_obj, "provider") and hasattr(
                        sparse_config_obj, "config"
                    ):
                        ...
                        logger.warning(
                            f"Unknown sparse_embedder config format: {type(sparse_config_obj)}. Expected SparseEmbedderConfig or dict with 'provider' and 'config' keys."
                        )
                        sparse_embedder_provider = None
                        config_dict = {}

                    if sparse_embedder_provider:
                        self.sparse_embedder = SparseEmbedderFactory.create(
                            sparse_embedder_provider, config_dict
                        )
                        logger.info(
                            f"Sparse embedder initialized: {sparse_embedder_provider}"
                        )
                except Exception as e:
        ...
        if sub_stores_list and self.storage_type.lower() == "oceanbase":
            ...
            self.storage = SubStorageAdapter(
                vector_store, self.embedding, self.sparse_embedder
            )
            logger.info("Using SubStorageAdapter with sub-store support")
        else:
            if sub_stores_list:
                logger.warning(
                    "The sub_stores function currently only supports oceanbase"
                )
        ...
        if self.memory_config:
            self.custom_fact_extraction_prompt = (
                self.memory_config.custom_fact_extraction_prompt
            )
            self.custom_update_memory_prompt = (
                self.memory_config.custom_update_memory_prompt
            )
        else:
            self.custom_fact_extraction_prompt = self.config.get(
                "custom_fact_extraction_prompt"
            )
            self.custom_update_memory_prompt = self.config.get(
        ...
        logger.info(
            f"Memory initialized with storage: {self.storage_type}, LLM: {self.llm_provider}, agent: {self.agent_id or 'default'}"
        )
    ...
    def _get_graph_enabled(self) -> bool:
        ...
        if self.memory_config:
            return (
                self.memory_config.graph_store.enabled
                if self.memory_config.graph_store
                else False
            )
        else:
            graph_store_config = self.config.get("graph_store", {})
            return (
                graph_store_config.get("enabled", False)
                if graph_store_config
                else False
            )
    ...
    def _get_intelligent_memory_config(self) -> Dict[str, Any]:
        ...
        if self.memory_config and self.memory_config.intelligent_memory:
            ...
            if self.memory_config.custom_importance_evaluation_prompt:
                cfg["custom_importance_evaluation_prompt"] = (
                    self.memory_config.custom_importance_evaluation_prompt
                )
            return cfg
        else:
            ...
            if "custom_importance_evaluation_prompt" in self.config:
                merged_cfg["custom_importance_evaluation_prompt"] = self.config[
    ...
    def _decide_memory_actions(
        ...
        try:
            ...
            for mem in existing_memories:
                ...
                old_memory.append({"id": mem.get("id", "unknown"), "text": content})

            # Generate update prompt with custom prompt if provided
            custom_prompt = None
            if (
                hasattr(self, "custom_update_memory_prompt")
                and self.custom_update_memory_prompt
            ):
                custom_prompt = self.custom_update_memory_prompt
            update_prompt = get_memory_update_prompt(
                old_memory, new_facts, custom_prompt
            )
        ...
        try:
            ...
            if llm_cfg.get("enable_vision"):
                messages = parse_vision_messages(
                    messages, self.llm, llm_cfg.get("vision_details"), self.audio_llm
                )
            else:
    ...
    def _simple_add(
        ...
        if not content or not content.strip():
            ...
            raise ValueError(
                f"Cannot create memory with empty content. Original messages: {messages}"
            )
        ...
        if self._intelligence_plugin and self._intelligence_plugin.enabled:
            extra_fields = self._intelligence_plugin.on_add(
        ...
        if enhanced_metadata and isinstance(enhanced_metadata, dict):
            ...
            enhanced_metadata = {
                k: v for k, v in enhanced_metadata.items() if k != "category"
            }

        # Final validation before storage
        if not content or not content.strip():
            raise ValueError(
                f"Refusing to store empty content. Original messages: {messages}"
            )
    ...
    def _intelligent_add(
        ...
        if not facts:
            ...
            if fallback_to_simple:
                logger.warning(
                    "No facts extracted from messages, falling back to simple add mode"
                )
                return self._simple_add(
                    messages,
                    user_id,
        ...
        for fact in facts:
            ...
            if metadata:
                ...
                simple_metadata = {
                    ...
                    and k not in ["agent_id", "user_id", "run_id"]
                }
        ...
        for mem in existing_memories:
            ...
            if mem_id and mem_id not in unique_memories:
                ...
                existing_distance = (
                    existing.get("distance", float("inf")) if existing else float("inf")
                )
        ...
        logger.info(
            f"Found {len(existing_memories)} existing memories to consider (after dedup and limiting)"
        )
        ...
        if facts:
            actions = self._decide_memory_actions(
                facts, existing_memories, user_id, agent_id
            )
        ...
        if not actions:
            ...
            if fallback_to_simple:
                logger.warning(
                    "No actions returned from LLM, falling back to simple add mode"
                )
                return self._simple_add(
                    messages,
                    user_id,
        ...
        for action in actions:
            ...
            logger.debug(
                f"Processing action: {event_type} - '{action_text[:50] if action_text else 'NONE'}...' (id: {action_id})"
            )
            ...
            try:
                if event_type == "ADD":
                    ...
                    if real_memory_id:
                        ...
                        logger.warning(
                            f"Could not find real memory ID for action ID: {action_id}"
                        )
        ...
        # If we processed actions but they were all NONE (duplicates detected), return empty results
            ...
            logger.info(
                "All actions were NONE (duplicates detected), returning empty results"
            )
    ...
    def search(
        ...
        try:
            ...
            if self.intelligence.enabled:
                processed_results = self.intelligence.process_search_results(
                    results, query
                )
            else:
            ...
            if self._intelligence_plugin and self._intelligence_plugin.enabled:
                updates, deletes = self._intelligence_plugin.on_search(
                    processed_results
                )
                    ...
                    try:
                        ...
                        if updates:
                            self.storage.update_memory(
                                memory_id, {**updates}, user_id, agent_id
                            )
                    except Exception:
                        pass
                # Track access count for analytics
    ...
    def update(
        ...
        try:
            ...
            if not content or not content.strip():
                raise ValueError(
                    f"Cannot update memory with empty content: '{content}'"
                )
            ...
            result = self.storage.update_memory(
                memory_id, update_data, user_id, agent_id
            )
    ...
    def reset(self):
        ...
        try:
            ...
            if hasattr(self.storage.vector_store, "reset"):
                ...
                vector_store_config = self._get_component_config("vector_store")
                self.storage.vector_store = VectorStoreFactory.create(
                    self.storage_type, vector_store_config
                )
                # Update storage adapter
                self.storage = StorageAdapter(
                    self.storage.vector_store, self.embedding, self.sparse_embedder
                )
    ...
    def _init_sub_stores(self):
        ...
        if self.storage_type.lower() != "oceanbase":
            logger.warning(
                f"Sub store feature only supports OceanBase storage, current storage: {self.storage_type}"
            )
        ...
        main_collection_name = (
            ...
            .get("config", {})
            .get("collection_name", "memories")
        )
        main_embedding_dims = (
            self.config.get("vector_store", {})
            .get("config", {})
            .get("embedding_model_dims", 1536)
        )

        # Iterate through configs and initialize each sub store
        for index, sub_config in enumerate(sub_stores_list):
            try:
                self._init_single_sub_store(
                    index, sub_config, main_collection_name, main_embedding_dims
                )
            except Exception as e:
    ...
    def _init_single_sub_store(
        ...
        sub_store_name = sub_config.get(
            "collection_name", f"{main_collection_name}_sub_{index}"
        )
        ...
        embedding_model_dims = sub_config.get(
            "embedding_model_dims", main_embedding_dims
        )

        # 4. Initialize sub store's embedding service
        sub_embedding_config = sub_config.get(
            "embedder", sub_config.get("embedding", {})
        )

        if sub_embedding_config:
            # Has independent embedding configuration
            sub_embedding_provider = sub_embedding_config.get(
            ...
            sub_embedding = EmbedderFactory.create(
                sub_embedding_provider, sub_embedding_params, sub_vector_config
            )
            logger.info(
                f"Created sub embedding service for store {index}: {sub_embedding_provider}"
            )
        else:
        ...
        if sub_vector_store_config:
            ...
            logger.info(
                f"Sub store {index} using custom vector_store config: {list(sub_vector_store_config.keys())}"
            )
        ...
        logger.info(
            f"Registered sub store {index}: {sub_store_name} (dims={embedding_model_dims})"
        )
    ...
    def migrate_to_sub_store(
        self, sub_store_index: int = 0, delete_source: bool = False
    ) -> int:
        """
        ...
        if isinstance(self.storage, SubStorageAdapter):
            migrated_count = self.storage.migrate_to_sub_store(
                store_name=sub_config["name"], delete_source=delete_source
            )
