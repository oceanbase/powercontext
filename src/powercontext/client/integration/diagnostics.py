...
def _state_path(host: str) -> Path:
    ...
    return root / "powercontext" / f"{host}-diagnostics.json"
...
def should_emit(host: str, outcome: str) -> bool:
...
class Events:
    """Buffer host-visible failures and emit content-free diagnostics."""
    ...
    def emit(self, outcome: str, *, event: str = "context_prepare", **details: object) -> None:
        import sys

        failure = outcome in _FAILURE_OUTCOMES
        if failure:
            if outcome in self.emitted:
                return
            self.emitted.add(outcome)
            if not should_emit(self.host, outcome):
                return
        record = {
            "component": f"powercontext.{self.host.replace('-', '_')}.recall",
            "event": event,
            "outcome": outcome,
    ...
    def write(self, context: str | None = None, *, include_empty: bool = False) -> None:
        import sys

        output: dict[str, object] = {}
        if self.failures:
            output["systemMessage"] = "\n".join(json.dumps(event, separators=(",", ":")) for event in self.failures)
        if context or include_empty:
