...
_SPECS: dict[RecordType, _RecordSpec] = {
    ...
    "skill_package": _RecordSpec(
        SKILL_PACKAGES_TABLE,
        ("scope_id", "tree_digest"),
        (
            "archive_digest",
            "archive_bytes",
            "manifest",
    ...
    "topic_memory_publication": _RecordSpec(
        TOPIC_MEMORY_REVISION_PUBLICATIONS_TABLE,
        ("scope_id", "family", "artifact_id", "revision"),
        ("published_at",),
    ),
    "profile_policy": _RecordSpec(
        PROFILE_POLICIES_TABLE,
        ("scope_id",),
        ("generation_enabled", "activation_mode", "pending_candidate_id", "version", "updated_at"),
    ),
    "artifact_tag": _RecordSpec(
        ARTIFACT_TAGS_TABLE,
...
class PortableBundleService:
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="powercontext-restore-") as directory:
            ...
            return await self._restore_snapshot(
                snapshot, supported_source_types=supported_source_types, progress=progress
            )
        ...
        *,
        ...
        for record_type in _EXPORT_ORDER:
            if record_type == "scope":
                continue
            for records in _staged_batches(staged, record_type):
                added, skipped = await self._write_batch(connection, record_type, records)
                inserted += added
                existing += skipped
    ...
    async def _write_batch(
        self, connection: AsyncConnection, kind: RecordType, records: tuple[_Record, ...]
    ) -> tuple[int, int]:
        spec = _SPECS[kind]
        keys = spec.identity
        identities = [
            tuple(_database_value(record.identity[key], spec.table.c[key]) for key in keys) for record in records
        ]
        rows = (
            await connection.execute(
                select(spec.table).where(tuple_(*(spec.table.c[key] for key in keys)).in_(identities))
            )
        ).mappings()
        present = {
            _identity_key(record.identity): record.digest
            for row in rows
            for record in (_record_from_row(kind, cast(Mapping[str, Any], dict(row))),)
        }
        values = []
        for record in records:
            ...
            if digest is not None:
                if digest != record.digest:
                    raise BundleConflictError(f"immutable identity conflict: {kind}")
            else:
...
def _row_values(record: _Record) -> dict[str, Any]:
    table = _SPECS[record.record_type].table
    values = {key: _database_value(value, table.c[key]) for key, value in record.identity.items()}
    values.update({key: _database_value(value, table.c[key]) for key, value in record.payload.items()})
    ...
    elif record.record_type == "skill_package":
...
def _staged_records(source: Path) -> Iterator[sqlite3.Connection]:
    """Index verified records on disk before acquiring the target write transaction."""
...
def _staged_type_records(staged: sqlite3.Connection, kind: RecordType) -> Iterator[_Record]:
    for row in staged.execute("SELECT document FROM records WHERE kind = ? ORDER BY sequence", (kind,)):
        yield _parse_record(json.loads(row[0]))


def _staged_batches(staged: sqlite3.Connection, kind: RecordType) -> Iterator[tuple[_Record, ...]]:
    batch: list[_Record] = []
    size = 0
    for row in staged.execute("SELECT document FROM records WHERE kind = ? ORDER BY sequence", (kind,)):
        batch.append(_parse_record(json.loads(row[0])))
        size += len(row[0])
        if len(batch) >= 100 or size >= 1024 * 1024:
            yield tuple(batch)
            batch.clear()
            size = 0
    if batch:
...
def _ordered_scope_records(records: Iterable[_Record], /) -> tuple[_Record, ...]:
    pending = {str(record.identity["scope_id"]): record for record in records}
...
def _validate_record_values(record: _Record, /) -> None:  # noqa: C901 - one versioned schema contract.
    ...
    if kind == "scope":
        ...
        integers("file_count", "uncompressed_size", "archive_size", positive=True)
        if not all(
            isinstance(value, str) and re.fullmatch(r"[0-9a-f]{64}", value)
            for value in (identity["tree_digest"], payload["archive_digest"])
        ):
            raise BundleFormatError("Skill package has invalid digest")
    ...
    elif kind == "topic_memory_publication":
        strings("published_at")
        _validate_datetime_text(payload["published_at"])
        if identity["family"] != "topic-memory":
            raise BundleFormatError("topic memory publication has invalid family")
    elif kind == "profile_policy":
        if not isinstance(payload["generation_enabled"], bool):
            raise BundleFormatError("profile policy has invalid generation state")
        strings("activation_mode", "pending_candidate_id", "updated_at", nullable=("pending_candidate_id",))
        _validate_datetime_text(payload["updated_at"])
        integers("version", positive=True)
        if payload["activation_mode"] not in {"automatic", "review_required"}:
            raise BundleFormatError("profile policy has invalid activation mode")
    elif kind == "artifact_tag":
        bytes_values("tag_key_hash")
        strings("tag", "assigned_at")
        _validate_datetime_text(payload["assigned_at"])
        if identity["target_type"] not in {"artifact", "memory_entry"}:
            raise BundleFormatError("artifact tag has invalid target type")
        if identity["target_type"] == "artifact" and identity["target_id"] != identity["artifact_id"]:
            raise BundleFormatError("artifact tag has invalid target")
        if identity["target_type"] == "memory_entry" and identity["family"] != "memory":
            raise BundleFormatError("artifact tag has invalid target")
...
def _validate_datetime_text(value: object, /) -> None:
    try:
        ...
        raise BundleFormatError("record payload has an invalid datetime field") from error
...
def _validate_record_dependencies(  # noqa: C901 - logical record kinds have distinct dependency edges.
    ...
    elif record.record_type == "topic_memory_publication":
        if not contains("artifact_revision", identity):
            raise BundleFormatError("topic memory publication references missing revision")
    elif record.record_type == "profile_policy":
        candidate_id = payload["pending_candidate_id"]
        if candidate_id is not None and not contains(
            "candidate_head", {"scope_id": scope, "candidate_id": candidate_id}
        ):
            raise BundleFormatError("profile policy references missing candidate")
    elif record.record_type == "artifact_tag":
        if not contains(
            "artifact_head",
            {"scope_id": scope, "family": identity["family"], "artifact_id": identity["artifact_id"]},
        ):
            raise BundleFormatError("artifact tag references missing artifact head")
        if identity["target_type"] == "memory_entry" and not _contains_memory_entry_target(record, contains):
            raise BundleFormatError("artifact tag references missing memory entry")
...
def _contains_memory_entry_target(
    record: _Record,
    contains: Callable[[str, Mapping[str, object]], bool],
    /,
) -> bool:
    return contains(
        "memory_entry_target",
        {
            ...
            "memory_artifact_id": record.identity["artifact_id"],
            "entry_id": record.identity["target_id"],
        },
    )
...
def _dependency_metadata(source: Path, /) -> tuple[dict[str, int], dict[tuple[str, str], tuple[str, int, int, int]]]:
    ...
    for record in _iter_records(source):
        ...
        if record.record_type != "skill_package":
            continue
        try:
            snapshot = capture_skill_archive(cast(bytes, _database_value(record.payload["archive_bytes"])))
        except SkillPackageError as error:
            raise BundleFormatError("Skill package archive is invalid") from error
        expected = {
            "tree_digest": record.identity["tree_digest"],
            **{
                field: record.payload[field]
                for field in ("archive_digest", "file_count", "uncompressed_size", "archive_size")
            },
        }
        if snapshot.reference.model_dump() != expected or snapshot.manifest_bytes != _database_value(
...
def _decoded_bytes_json(value: object, name: str, /) -> object:
    ...
    if not isinstance(encoded, str):
        raise BundleFormatError(f"{name} is not encoded bytes")
    try:
        return json.loads(base64.b64decode(encoded, validate=True), parse_constant=_reject_json_constant)
    ...
    try:
        ...
        for record in _iter_records(source):
            ...
            if record.record_type == "memory_entry_version":
                ...
                connection.execute(
                    "INSERT OR IGNORE INTO identities (record_type, identity) VALUES (?, ?)",
                    (
                        "memory_entry_target",
                        _identity_key({
                            "scope_id": record.identity["scope_id"],
                            "memory_artifact_id": record.identity["memory_artifact_id"],
                            "entry_id": record.payload["entry_id"],
                        }),
                    ),
                )
        connection.commit()

        def contains(record_type: str, identity: Mapping[str, object]) -> bool:
            return (
                connection.execute(
...
def _required_artifact_families(records: Iterable[_Record]) -> set[str]:
    ...
    for record in records:
        ...
        if record.record_type == "profile_policy":
...
def _database_value(value: object, column: Any | None = None) -> object:
    encoded = cast(dict[str, object], value).get("base64") if isinstance(value, dict) else None
    ...
    if isinstance(value, str) and column is not None and isinstance(column.type, DateTime):
        try:
            return datetime.fromisoformat(value)
        except ValueError as error:
            raise BundleFormatError("invalid datetime column value") from error
    return value
