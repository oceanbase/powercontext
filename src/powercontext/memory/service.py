...
class MemoryService(ArtifactCatalog[Memory]):
    ...
    async def search(
        ...
        *,
        ...
        selected = tuple(dict.fromkeys(memories))
        selected_memories = tuple(memory.ref for memory in selected)
    ...
    async def _validate_search_heads(self, memories: tuple[Memory, ...]) -> None:
        for memory in memories:
            exact = await self._backend.get(memory.ref)
            latest = await self._backend.latest(memory.artifact_id)
            if exact != memory or exact.ref != latest.ref:
    ...
    def _deduplicate_manifest(
        ...
        for entry_ids in groups.values():
            ...
            for entry_id in ordered[1:]:
                ...
                changes.append(MemoryChange("deactivate", entry_id, item.entry_version_id, None, "dedupe"))
    ...
    def _normalize_manifest_entries(
        ...
        *,
        ...
        for entry_id, previous in tuple(current_by_entry.items()):
            ...
            changes.append(
                MemoryChange("revise", entry_id, previous.entry_version_id, version.entry_version_id, "normalize")
            )
    ...
    async def _previous_projections(self, base: Memory | None) -> dict[str, MemoryProjection]:
        ...
        return {
            ...
            for projection in await self._backend.projections(base.ref)
        }
    ...
    async def _canonical_operation_evidence(
        ...
        return _OperationEvidence(tuple(canonical_sources), tuple(canonical_artifacts))

    async def _validated_entries(self, memory: Memory) -> tuple[MemoryEntryVersion, ...]:
        versions = await self._backend.entries(memory.ref)
        ...
        *,
        ...
        for candidate in candidates:
            ...
            material = await self._material_from_candidate(
                candidate,
                (*previous.sources, *evidence.sources),
                (*evidence.artifacts,),
                previous=previous,
            )
        ...
        memory = Memory(
            ...
            lineage=ArtifactLineage(
                sources=evidence.sources,
                artifacts=tuple(artifact.ref for artifact in evidence.artifacts),
            ),
        )
        projections = await self._prepare_projections(
            base=base,
    ...
    async def _material_from_candidate(
        ...
        *,
        ...
        if previous is None:
            ...
            sources = (*previous.sources, *candidate_sources)
    ...
    async def _canonical_candidate_artifacts(
        ...
        allowed_refs = tuple(artifact.ref for artifact in allowed)
        for value in values:
            canonical = value if self._artifact_resolver is None else await self._artifact_resolver.get(value)
            if canonical.ref not in (*allowed_refs, *previous):
                raise InvalidMemoryEvidenceError("artifact-outside")
            if canonical.ref not in result:
                result.append(canonical.ref)
        return tuple(result)
    ...
    def _entry_material(
        ...
        *,
        ...
        canonical_sources, source_refs = self._source_refs(sources)
        canonical_artifacts, artifact_refs = self._artifact_refs(artifacts)
    ...
    def _source_refs(self, sources: Sequence[Source]) -> tuple[tuple[Source, ...], tuple[object, ...]]:
        if not sources:
            return (), ()
        if self._evidence_codec is None:
            raise InvalidMemoryEvidenceError("source-codec")
        keyed: dict[bytes, tuple[Source, object]] = {}
        for source in sources:
            reference = self._evidence_codec.encode_source(source)
            keyed.setdefault(canonical_json(reference), (source, reference))
        keys = sorted(keyed)
        return tuple(keyed[key][0] for key in keys), normalize_refs(tuple(keyed[key][1] for key in keys))

    def _artifact_refs(
        self,
        artifacts: Sequence[ArtifactRef],
    ) -> tuple[tuple[ArtifactRef, ...], tuple[object, ...]]:
        if not artifacts:
            return (), ()
        if self._evidence_codec is None:
            raise InvalidMemoryEvidenceError("artifact-codec")
        keyed: dict[bytes, tuple[ArtifactRef, object]] = {}
        for artifact in artifacts:
            reference = self._evidence_codec.encode_artifact(artifact)
            keyed.setdefault(canonical_json(reference), (artifact, reference))
        keys = sorted(keyed)
        return tuple(keyed[key][0] for key in keys), normalize_refs(tuple(keyed[key][1] for key in keys))
