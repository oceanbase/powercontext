...
"""Map workbuddy prompt events to the installed client's shared hook."""
...
if (_PLUGIN_ROOT / "hooks/powercontext_scope_binding.py").is_file():
...
def main(settings: WorkBuddyPluginSettings | None = None) -> int:
    ...
    try:
        ...
        if not isinstance(query, str) or not query.strip() or not isinstance(cwd, str):
            events.emit("skipped")
            events.write(include_empty=True)
            return 0
        ...
        events.write(scope_context(context, scope_id), include_empty=True)
    except Exception:
        return 0
    return 0
