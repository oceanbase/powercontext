...
def install_dsh_plugin(*, source: str, ref: str, server_url: str = "http://127.0.0.1:8000") -> DshSetupResult:
    ...
    host_adapter("dsh").prepare(plugin_dir)
    require_built_plugin(plugin_dir)
    _run_dsh("plugin", "--profile", DSH_PROFILE, "add", "--workspace-root", str(plugin_dir))
...
def plugin_dir_from_checkout(root: Path) -> Path:
    ...
    if _is_dsh_plugin(plugin):
        return plugin
    raise SetupError.not_found("PowerContext DSH plugin", root)
...
def require_built_plugin(path: Path) -> None:
    ...
    if not (path / DSH_BUNDLE).is_file():
        raise SetupError(f"PowerContext DSH plugin at {path} is missing lib/index.js. Build the plugin before setup.")
...
def dsh_executable() -> str:
    ...
    if executable is None:
        raise SetupError.unavailable("DeepSeek Harness CLI")
    return executable
...
def github_clone_url(source: str) -> str:
    ...
    try:
        ...
        raise SetupError.invalid_source("DeepSeek Harness") from None
...
def checkout_target(ref: str) -> Path:
    ...
    if not ref or ref in {".", ".."} or "\x00" in ref:
        raise SetupError.invalid_ref("DeepSeek Harness", ref)
    target = (root / ref).resolve()
    try:
        target.relative_to(root)
    except ValueError as error:
        raise SetupError.invalid_ref("DeepSeek Harness", ref) from error
    if target == root:
        raise SetupError.invalid_ref("DeepSeek Harness", ref)
    return target
