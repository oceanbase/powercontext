...
def install_hermes_plugin(*, source: str, ref: str) -> HermesSetupResult:
    ...
    try:
        ...
        raise SetupError(f"Cannot install PowerContext Hermes plugin at {target}: {error}") from error
...
def _stage_hermes_plugins(
    ...
    try:
        for source_dir, target in source_dirs:
            ...
            if target.name == HERMES_PLUGIN_NAME:
                host_adapter("hermes").prepare(staging)
...
def plugin_dir_from_checkout(root: Path) -> Path:
    ...
    if _is_hermes_plugin(root):
        return root
    distributed = root / "plugins" / HERMES_PLUGIN_NAME
    if _is_hermes_plugin(distributed):
        return distributed
    plugin = root / HERMES_PLUGIN_RELATIVE
    if _is_hermes_plugin(plugin):
        return plugin
    raise SetupError.not_found("PowerContext Hermes plugin", root)
...
def hermes_executable() -> str:
    ...
    if executable is None:
        raise SetupError.unavailable("Hermes CLI")
    return executable
...
def checkout_target(source: str, ref: str) -> Path:
    ...
    if not ref or ref in {".", ".."} or "\x00" in ref:
        raise SetupError.invalid_ref("Hermes", ref)
    source_key = _source_cache_key(source)
    target = (root / source_key / ref).resolve()
    try:
        target.relative_to(root)
    except ValueError as error:
        raise SetupError.invalid_ref("Hermes", ref) from error
    if target == root:
        raise SetupError.invalid_ref("Hermes", ref)
    return target
...
def github_clone_url(source: str) -> str:
    ...
    if "://" in text or text.startswith("git@"):
        raise SetupError.invalid_source("Hermes")
...
def get_hermes_version(executable: str | None = None) -> str:
    ...
    if parsed < HERMES_MIN_VERSION:
        ...
        raise SetupError(
            f"Hermes Agent v{actual} is unsupported; PowerContext requires Hermes Agent v{minimum} or newer."
        )
