...
def resolve_host_transport(host: str) -> tuple[str, bool]:
    ...
    """
    if host == "minimax":
        from .minimax import resolve_minimax_connection

        connection = resolve_minimax_connection()
        return connection.base_url, connection.allow_insecure_http
    if host == "dsh":
        _check_dsh_overlays()
...
def resolve_host_connection(host: str):
    """Supply native configuration to the shared Server diagnostic flow."""
    from powercontext.client.integration.models import Connection

    from .authorization import credential_path, read_stored_authorization
    from .host import host_adapter

    if host == "minimax":
        from .minimax import resolve_minimax_connection

        return resolve_minimax_connection()
    endpoint, allowed = resolve_host_transport(host)
    settings = host_adapter(host).target.setup.settings
    prefix = settings.get("environment_prefix", "POWERCONTEXT_" + host.upper().replace("-", "_"))
    key = settings.get("authorization_setting", "AUTHORIZATION")
    authorization = os.environ.get(f"{prefix}_{key}") if prefix and key else None
    if not authorization:
        with suppress(ValueError):
            authorization = read_stored_authorization(credential_path(host), server_url=endpoint).authorization
    native, _, _ = _native_settings(host)
    authorization = (
        authorization
        or native.get("authorization")
        or native.get("token")
