...
def install_pi_plugin(*, source: str, ref: str, server_url: str = "http://127.0.0.1:8000") -> PiSetupResult:
    ...
    host_adapter("pi").prepare(package_dir)
...
def plugin_dir_from_checkout(root: Path) -> Path:
    ...
    if _is_pi_package(package):
        return package
    raise SetupError.not_found("PowerContext Pi package", root)
...
def require_pi_package(path: Path) -> None:
    ...
    if not (path / PI_EXTENSION).is_file() or not (path / PI_SKILL).is_file():
        raise SetupError(
            f"PowerContext Pi package at {path} is missing its extension or powercontext-project-context skill."
        )
...
def pi_executable() -> str:
    ...
    if executable is None:
        raise SetupError.unavailable("Pi CLI")
    return executable
...
def github_clone_url(source: str) -> str:
    ...
    try:
        ...
        raise SetupError.invalid_source("Pi") from None
...
def checkout_target(ref: str) -> Path:
    ...
    if not ref or ref in {".", ".."} or "\x00" in ref:
        raise SetupError.invalid_ref("Pi", ref)
    candidate = (root / ref).resolve()
    try:
        candidate.relative_to(root)
    except ValueError as error:
        raise SetupError.invalid_ref("Pi", ref) from error
    if candidate == root:
        raise SetupError.invalid_ref("Pi", ref)
    return root / "current"
