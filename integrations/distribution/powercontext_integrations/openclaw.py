...
def install_openclaw_plugin(
    *,
    ...
    host_adapter("openclaw").prepare(plugin_dir)
...
def openclaw_executable() -> str:
    ...
    for candidate in candidates:
        ...
        if executable is not None:
            return executable
    raise SetupError.unavailable("OpenClaw CLI")
...
def pnpm_executable() -> str:
    ...
    for candidate in candidates:
        ...
        if executable is not None:
            return executable
    raise SetupError("pnpm is not installed or is not on PATH; it is required to build the OpenClaw plugin.")
...
def require_supported_openclaw(executable: str) -> str:
    ...
    if match is None:
        raise SetupError(f"OpenClaw {version_text or 'version unknown'} is unsupported; upgrade to >= 2026.8.1-beta.2")
...
def plugin_dir_from_checkout(root: Path) -> Path:
    ...
    if is_openclaw_plugin(plugin):
        return plugin
    raise SetupError.not_found("PowerContext OpenClaw plugin", root)
...
def checkout_target(ref: str) -> Path:
    ...
    if not ref or ref in {".", ".."} or "\x00" in ref:
        raise SetupError.invalid_ref("OpenClaw", ref)
    root = (powercontext_data_dir() / "checkouts" / OPENCLAW_CHECKOUT_ROOT).resolve()
    target = (root / ref).resolve()
    try:
        target.relative_to(root)
    except ValueError as error:
        raise SetupError.invalid_ref("OpenClaw", ref) from error
    if target == root:
        raise SetupError.invalid_ref("OpenClaw", ref)
    return target
...
def github_clone_url(source: str) -> str:
    ...
    if "://" in text or text.startswith("git@"):
        raise SetupError.invalid_source("OpenClaw")
...
def build_openclaw_plugin(plugin_dir: Path) -> None:
    ...
    if not (plugin_dir / "dist" / "index.js").is_file():
        raise SetupError(f"PowerContext OpenClaw plugin at {plugin_dir} is missing dist/index.js after build.")
...
def normalize_server_url(value: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or parsed.hostname is None:
        raise SetupError("OpenClaw PowerContext Server URL must use HTTP or HTTPS")
    if parsed.username is not None or parsed.password is not None:
        raise SetupError("OpenClaw PowerContext Server URL must not contain credentials")
    if parsed.query or parsed.fragment:
        raise SetupError("OpenClaw PowerContext Server URL must not contain a query or fragment")
