...
Choose one method to install the CLI and Server. If you already have uv, install directly.
Otherwise, use the script for your platform.

<Tabs items={["uv", "macOS / Linux script", "Windows PowerShell (experimental)"]}>

<Tab value="uv">

Use [`uv`](https://docs.astral.sh/uv/) to install PowerContext from PyPI. PowerContext requires Python 3.11+.
...
If you need to test this version from source, use uv and Git to install its tag. For everyday use, install the PyPI release above:
...
including credential helpers and SSH settings. For an SSH install, replace the HTTPS URL with the Git URL
approved for your environment. `--force` refreshes an existing tool from the current commit behind the selected
...
Agent integrations should use the same ref as the Server.

</Tab>

<Tab value="macOS / Linux script">

You need Bash and curl. The script reuses existing uv and Python 3.11+ installations and downloads missing components.

```bash
curl -fsSL https://powercontext.oceanbase.io/install.sh -o install.sh
bash install.sh --version 1.0.0 --no-hosts
```

After installation, apply the `PATH` command printed by the script to your terminal.

<h3>Script options and download sources</h3>

The scripts default to `1.0.0`. Use `--version <version>` for another release,
replace `--no-hosts` with `--host codex` to include the Codex integration, or use `--help` for all options.

The scripts reuse local uv and Python 3.11+ installations. Missing uv is installed at `0.12.12`;
missing Python is installed at 3.12. Existing uv installations are not upgraded.

Use `--region cn` or `--region global` to choose a download region. The default is `auto`.
Priority is the command-line option, `POWERCONTEXT_INSTALL_REGION`, timezone, locale territory, then `global`.
For example, `Asia/Shanghai` selects CN; with a UTC timezone, `zh_CN` or `zh-CN` also selects CN.
A UTC+8 offset, the abbreviation CST, or a Chinese language preference alone does not establish a region.
The scripts do not query an IP location service.

CN prefers Tsinghua for PyPI. Tsinghua does not provide the uv and Python runtime artifacts used here,
so those use USTC and NJU respectively. Other regions use PyPI and uv official download channels.
The uv installer and binaries use the same version directory. Before downloading Python, the script checks
that the mirror provides the build selected by uv. Unreachable mirrors or missing versions fall back to official sources.

Existing uv configuration is preserved. Pass `--index-url https://pypi.tuna.tsinghua.edu.cn/simple` to choose a package
index without index fallback. Explicit download settings take precedence over regional defaults and fail without fallback.
Automatic settings apply only to this installation, without writing global configuration. Errors after installation starts
and dependency installation failures do not trigger a retry of the entire workflow.

uv and Python themselves use separate download servers. `POWERCONTEXT_UV_INSTALLER_URL` selects a platform-matching
uv installer (`.sh`); `UV_INSTALLER_GITHUB_BASE_URL` and `UV_PYTHON_INSTALL_MIRROR` configure GitHub-style uv and Python download mirrors.
Use `UV_DOWNLOAD_URL` to specify a directory containing uv release artifacts.

</Tab>

<Tab value="Windows PowerShell (experimental)">

Windows support is experimental and requires Windows PowerShell 5.1 or newer.
The script reuses existing uv and Python 3.11+ installations and downloads missing components.

```powershell
Invoke-WebRequest https://powercontext.oceanbase.io/install.ps1 -UseBasicParsing -OutFile install.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File .\install.ps1 --version 1.0.0 --no-hosts
```
...
If installing uv from the automatic mirror fails, the PowerShell script retries that step once with official sources.

uv and Python themselves use separate download servers. `POWERCONTEXT_UV_INSTALLER_URL` selects a platform-matching
uv installer (`.ps1`); `UV_INSTALLER_GITHUB_BASE_URL` and `UV_PYTHON_INSTALL_MIRROR` configure GitHub-style uv and Python download mirrors.
Use `UV_DOWNLOAD_URL` to specify a directory containing uv release artifacts.
...
After installation, [generate a Server environment file](configure-server-environment.md).
[Configure models](configure-models.md) if you need extraction and vector search.
Agent integrations also require Git. Follow the [integration guides](../integrations/index.md) and use the same version as the Server.
