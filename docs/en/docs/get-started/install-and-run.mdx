...
The scripts use existing uv configuration. Without it, they try the Tsinghua mirror first when the network exit country
is CN, and PyPI first elsewhere or when country detection fails. If an index is unreachable or does not list the requested
PowerContext version, they try the other index. Pass `--index-url https://pypi.tuna.tsinghua.edu.cn/simple` to select a
mirror without automatic fallback. The scripts do not write global index configuration or retry with another index
if dependency installation fails.

uv and Python themselves use separate download servers. `POWERCONTEXT_UV_INSTALLER_URL` selects a platform-matching
uv installer (`.sh`); `UV_INSTALLER_GITHUB_BASE_URL` and `UV_PYTHON_INSTALL_MIRROR` configure uv binary and Python download mirrors.
...
uv and Python themselves use separate download servers. `POWERCONTEXT_UV_INSTALLER_URL` selects a platform-matching
uv installer (`.ps1`); `UV_INSTALLER_GITHUB_BASE_URL` and `UV_PYTHON_INSTALL_MIRROR` configure uv binary and Python download mirrors.
