...
bash install.sh --version 1.0.0rc2 --no-hosts
```
...
The scripts default to `1.0.0rc2`. Use `--version <version>` for another release,
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\install.ps1 --version 1.0.0rc2 --no-hosts
```
