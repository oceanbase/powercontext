...
Home memories use Tabler Card and List Group. Native grid and Flex utilities align the top and bottom card edges with the experiences and skills section. From xl, existing memory rows divide the card height equally; narrower screens use natural content heights. Memory navigation, reading content and experience/skill lists use independently scrollable regions with stable heights. Pagination stays at the bottom of its region when item counts or text lengths change.
