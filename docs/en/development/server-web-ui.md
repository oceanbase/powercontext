...
The Handoff Report page obtains Projects from `POST /v1/handoff-reports/projects/list` and uses immutable `project_id` values in its Project selector. Each option shows both the Project title and full `project_id` so duplicate titles remain distinguishable. Selecting a Project requests canonical JSON through `POST /v1/handoff-reports/get`; the Handoff work selector then lists only Workstreams registered to that Project. The browser formats and displays the returned `summary`, `coverage`, Workstream state, Activity count, objective, current state, next action, known omissions, and digests without recalculating report semantics.
