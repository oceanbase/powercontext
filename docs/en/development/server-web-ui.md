...
The Handoff Report page obtains Projects from `POST /v1/handoff-reports/projects/list` and uses immutable `project_id` values in a searchable Project combobox. Search matches Project title, `project_id`, or Project key; each option shows the title and full `project_id`, and the browser renders at most the first 50 matches with a prompt to narrow the query. Selecting a Project loads only that Project's Workstreams and requests canonical JSON through `POST /v1/handoff-reports/get`. The report presents those Workstreams in a horizontal switcher above the full-width current Handoff snapshot, without a second selector. When the switcher overflows, previous and next controls expose its current position and keep the selected Workstream visible. Projects with more than eight Workstreams also receive a title-or-scope search field.
...
When the Project catalog succeeds but contains no active Projects, the page replaces the report controls with a clearly labeled, data-free template preview. Preview values use an em dash (`—`); Project search, period filters, Markdown download, and Handoff editing are unavailable because there is no Project to query or update. Retry reloads the catalog, and the first available Project replaces the preview. The preview neither creates a Project nor requests fabricated report data.
