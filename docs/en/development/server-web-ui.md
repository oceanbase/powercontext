...
When the Project list succeeds but contains no active Projects, the page shows a clearly labeled, data-free template preview instead of an empty state. The preview explains the Project summary and scope, coverage, Workstream and Handoff status, continuation content, and Activity period comparison; every value is shown as an em dash (`—`). Retrying reloads the Project catalog, and the first available real report replaces the preview. The preview never creates a Project or requests a fabricated report.
