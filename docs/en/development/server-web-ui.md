...
The Handoff Report page obtains Projects from `POST /v1/handoff-reports/projects/list` and uses immutable `project_id` values in its Project selector. Each option shows both the Project title and full `project_id` so duplicate titles remain distinguishable. Selecting a Project requests canonical JSON through `POST /v1/handoff-reports/get`. The report then presents that Project's Workstreams in a horizontal switcher above the full-width current Handoff snapshot, without a second selector. When the switcher overflows, previous and next controls expose its current position and keep the selected Workstream visible. Projects with more than eight Workstreams also receive a title-or-scope search field. The sender and receiver workspace is a secondary action inherited from the selected Workstream, with an explicit close action in its heading. Activity period controls stay with the Activity section because they affect Activity counts only. The browser formats the returned `summary`, `coverage`, Workstream state, Activity count, objective, current state, next action, known omissions, and digests without recalculating report semantics.
