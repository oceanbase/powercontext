...
- Related RFC: [RFC 0003: Memory Layer Design](0003_memory_layer_design.md)
...
RFC 0003 already defines Memory versioning, lineage, writes, and retrieval, but it leaves two model entry points
unspecified:
...
persistence and consistency rules from RFC 0003.
...
must use the same profile. Changing the embedding model or dimension still follows the RFC 0003 process: pause writes,
...
2. Consolidation of the existing embedding port into the generic model capability while preserving RFC 0003 behavior.
...
- Model failure creates no partial Revision, and embedding failure degrades according to RFC 0003.
