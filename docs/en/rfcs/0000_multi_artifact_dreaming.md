...
Generated Clients use closed Dream enums and Candidate proposal unions. The minimum supported feature level is **Dream contract 2**. The coordinated Python Client, CLI, MCP bridge, and Dashboard send `X-PowerContext-Dream-Contract: 2`; external integrations must implement and declare the same contract. This feature level is independent of package release numbering, including development snapshots. Responses advertise the minimum level. Clients without the header retain the original operations and see only `refine_experience` and `derive_skill` in capabilities. Reading an extended Run, Candidate, or `dream_review_approved` Profile returns HTTP 426 `client_upgrade_required`; mutations using new types are rejected before a write. Compatible responses omit new optional audit fields without disguising operation, mode, or Source windows. Test both the coordinated SDK and a legacy client without the header. Tag proposals retain separate Candidate interfaces and their ETag result lifecycle. Trusted evaluation infrastructure remains outside this PR.

For Artifact operations, `target` remains an exact ArtifactRef and must also appear in `artifacts`. Memory entry identities and versions are supplied through `memory_citations`, all belonging to that target Memory. References are deduplicated and counted against the evidence budget; a target cannot independently support its own claims. For Profile, Topic Memory, and Handoff, validate the target permission, existence, and exact head separately: retired historical dependencies must not prevent correction using new valid evidence. Recursively validate all selected supporting evidence without granting it the target exemption. Explicit Sources and references must belong to the same Scope. Stage C expresses TagTarget, expected ETag, and the exact content basis through a separate `tag_target` object on the existing Dream request, without changing the Artifact `target` type. Its result uses `candidate.kind=catalog_change` to identify the separate Catalog Change Candidate resource.
...
## Review Inbox presentation

The Dashboard combines the two public Candidate services with `all/artifact/catalog_change` and status filters. Each resource uses its own cursor, with 12 candidates per section and independent next-page links; the combined view does not claim a global chronological order. Detail pages show the exact target diff, complete proposal, evidence, and decision history, with explicit Prompt publication, Skill non-installation, and Handoff non-activation effects. Approve, reject, and revise submit the reviewed version through the same authorized HTTP interfaces. Browser mutations require same-origin requests. No aggregate table or Inbox API is introduced.
...
The idempotency key handles request replay. Candidate suppression additionally fingerprints operation, complete target baseline, root evidence digest, and policy within one Scope and one principal. Before reusing a pending Candidate, verify its current version, actual content and evidence, and permission; editing the proposal invalidates an outdated fingerprint. Do not reuse across principals in the first release. A rejected proposal with unchanged evidence yields `no_change` and a readable reference to the earlier decision, but cannot conceal new counterevidence or a changed target.
...
1. Coordinate external integration releases against the defined Dream contract 2 feature level.
...
4. Consider cross-resource chronological sorting only when needed; the initial Inbox uses resource filters and independent cursors.
