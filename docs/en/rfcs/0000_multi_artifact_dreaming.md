...
For new operations, `target` is a discriminated value: an exact ArtifactRef; for Memory, the owning Memory Ref and exact Entry Version identities; for Tag, a TagTarget, expected ETag, and exact content basis. The target contributes to the bounded model context and evidence budget but is not independent proof of its own claims. Explicit `artifacts`, `memory_citations`, and `sources` remain exact, same-Scope references. A target cannot be `latest`, a Candidate, or another DreamRun.

The existing idempotency identity remains `(scope_id, principal_id, idempotency_key)`, stored using the current principal key digest. The same normalized request returns its original run; changed input under the same key conflicts. Different principals may use the same key without sharing a run, ownership, or worker identity. On first execution freeze the operation adapter, policy, model, and server prompt version for retries. Existing HTTP 202/200 replay semantics remain.
