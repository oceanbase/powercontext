...
`POWERCONTEXT_SERVER_PUBLIC_URL`。该值仅声明外部可达地址，不会修改 `HTTP_HOST` 或 `HTTP_PORT`；直连时端口应与监听端口一致，使用反向代理时则填写代理入口。
Skills Dashboard 会自动用它生成注册命令，不再要求每次添加目标时填写地址。
