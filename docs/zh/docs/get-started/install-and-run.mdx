...
脚本沿用已有 uv 配置。没有配置时，网络出口地区为 CN 则先尝试清华源，其他地区或地区检测失败时先尝试 PyPI。
如果源无法访问或尚未同步指定的 PowerContext 版本，脚本会尝试另一个源。
传入 `--index-url https://pypi.tuna.tsinghua.edu.cn/simple` 可指定镜像并关闭自动回退。
脚本不写入全局源配置；依赖安装阶段失败时不会自动换源重试。

uv 和 Python 本体使用独立的下载地址。`POWERCONTEXT_UV_INSTALLER_URL` 可指定与平台对应的 uv 安装脚本
（`.sh`）；`UV_INSTALLER_GITHUB_BASE_URL` 和 `UV_PYTHON_INSTALL_MIRROR` 分别配置 uv 二进制和 Python 的下载镜像。
...
uv 和 Python 本体使用独立的下载地址。`POWERCONTEXT_UV_INSTALLER_URL` 可指定与平台对应的 uv 安装脚本
（`.ps1`）；`UV_INSTALLER_GITHUB_BASE_URL` 和 `UV_PYTHON_INSTALL_MIRROR` 分别配置 uv 二进制和 Python 的下载镜像。
