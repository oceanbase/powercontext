...
选择一种方式安装 CLI 和 Server。已有 uv 时可以直接安装；没有 uv 或 Python 时，选择对应平台的脚本。

<Tabs items={["uv", "macOS / Linux 脚本", "Windows PowerShell（实验性）"]}>

<Tab value="uv">

使用 [`uv`](https://docs.astral.sh/uv/) 从 PyPI 安装 PowerContext。PowerContext 需要 Python 3.11+。
...
如果需要测试当前版本的源码，可以使用 uv 和 Git 安装对应 tag。日常使用建议安装上面的 PyPI 发布包：
...
如需使用 SSH，请把 HTTPS URL 换成当前环境允许的 Git URL。`--force` 会从所选 Git ref 当前指向的 commit
刷新已安装工具；如果不加该参数，uv 可能只提示相同 requirement 已安装，而不会获取更新后的 `master`。

安装其他分支或 tag 时，替换最后一个 `@` 后的 ref。`master` 分支可能包含尚未发布的改动。
Agent 集成也应使用与 Server 相同的 ref。

</Tab>

<Tab value="macOS / Linux 脚本">

需要 Bash 和 curl。脚本会复用已有的 uv 和 Python 3.11+，只下载缺少的组件。
...
安装后，按脚本输出的命令设置当前终端的 `PATH`。

<h3>脚本选项与下载源</h3>

脚本默认安装 `1.0.0`。通过 `--version <version>` 选择其他发布版，
将 `--no-hosts` 换成 `--host codex` 可同时安装 Codex 集成，完整参数见 `--help`。

脚本先复用本地 uv 和 Python 3.11+。只有缺少组件时才下载：新安装的 uv 固定为 `0.12.12`，
缺少兼容 Python 时安装 3.12。已有 uv 不会自动升级。

使用 `--region cn` 或 `--region global` 指定下载地区，默认 `auto`。选择顺序为命令行参数、
`POWERCONTEXT_INSTALL_REGION`、时区、语言中的地区，最后使用 `global`。
例如 `Asia/Shanghai` 选择 CN；时区为 UTC 时，`zh_CN` 或 `zh-CN` 也会选择 CN。
仅有 `UTC+8`、`CST` 或中文语言偏好不足以判断地区。脚本不请求 IP 定位服务。

CN 优先使用清华 PyPI；清华未提供的 uv 和 Python 运行时分别使用中科大和南京大学镜像。
其他地区使用 PyPI 和 uv 官方下载渠道。uv 安装脚本及二进制使用相同版本目录，
Python 下载前检查镜像是否提供 uv 选定的构建；镜像无法访问或缺少版本时回退到官方渠道。

脚本沿用已有 uv 配置。传入 `--index-url https://pypi.tuna.tsinghua.edu.cn/simple` 可指定包索引并关闭索引回退。
显式配置的下载地址优先于地区默认值；它们失败时直接报错。所有自动设置仅对本次安装生效，
不会写入全局配置；开始安装后的错误和依赖安装失败不会触发整个流程重试。

uv 和 Python 本体使用独立的下载地址。`POWERCONTEXT_UV_INSTALLER_URL` 可指定与平台对应的 uv 安装脚本
（`.sh`）；`UV_INSTALLER_GITHUB_BASE_URL` 和 `UV_PYTHON_INSTALL_MIRROR` 分别配置 GitHub 形式的 uv 下载源和 Python 镜像。
如需指定包含 uv 发布产物的目录，可使用 `UV_DOWNLOAD_URL`。

</Tab>

<Tab value="Windows PowerShell（实验性）">

Windows 支持为实验性，需要 Windows PowerShell 5.1 或更新版本。
脚本会复用已有的 uv 和 Python 3.11+，只下载缺少的组件。
...
通过自动镜像安装 uv 失败时，PowerShell 脚本会使用官方渠道重试这一步。

uv 和 Python 本体使用独立的下载地址。`POWERCONTEXT_UV_INSTALLER_URL` 可指定与平台对应的 uv 安装脚本
（`.ps1`）；`UV_INSTALLER_GITHUB_BASE_URL` 和 `UV_PYTHON_INSTALL_MIRROR` 分别配置 GitHub 形式的 uv 下载源和 Python 镜像。
如需指定包含 uv 发布产物的目录，可使用 `UV_DOWNLOAD_URL`。
...
安装完成后，[生成 Server 环境文件](configure-server-environment.md)，
需要提取与向量搜索时再[配置模型](configure-models.md)。
安装 Agent 集成还需要 Git，步骤见[各自的集成文档](../integrations/index.md)；集成应与 Server 使用同一版本。
