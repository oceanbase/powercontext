...
[HTTP API 生命周期教程](../develop/api-quickstart.md)，直接跑通第一个受治理的上下文闭环。
...
[配置 Agent Plugin](../integrations/agent-plugin.md)加载
...
不要仅因为写入 Handoff 就宣称任务已经完成。详细激活和命令边界见[配置 Hermes](../integrations/hermes.md)。
...
分别阅读：[Pydantic AI](../integrations/pydantic-ai.md)、[LangChain](../integrations/langchain.md)和
[LangGraph](../integrations/langgraph.md)。Bub package 的说明位于 `integrations/bub/README.md`。
...
继续深入时，可选择：[Codex 完整分步教程](../workflows/codex-workflow.md)、
[Memory 与 Handoff](../workflows/memory-and-handoff.md)、[完整工作交接](../workflows/handoff-with-codex.md)、
[完整功能 Quick Start](configure-models.md)、[部署 Server](../operate/deploy-server.md)或
