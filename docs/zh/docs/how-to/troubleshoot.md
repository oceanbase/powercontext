...
5. 使用 OceanBase `obloader` 只把导出的数据导入已经存在的新表，同样**不要使用 `--ddl`**。导入旧 DDL 会重新
   创建不兼容的 collation。
6. 比较记录数、检查 identity column collation，并测试仅大小写或重音不同的 identity。全部检查通过后再恢复正常
   流量。在回滚窗口结束之前，保留旧数据库和备份。
