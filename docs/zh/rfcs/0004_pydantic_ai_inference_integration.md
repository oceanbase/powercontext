...
- 相关 RFC：[RFC 0003：Memory Layer 设计](0003_memory_layer_design.md)
...
RFC 0003 已经定义 Memory 的版本、血缘、写入和检索能力，但还缺少两个模型入口：
...
因此“结构化输出有效”不等于“允许保存”。本 RFC 不改变 RFC 0003 的 Memory 持久化和一致性规则。
...
distance 和 normalization；查询与 Memory projection 必须使用相同 profile。更换 embedding 模型或维度仍按 RFC 0003
...
2. 将现有 embedding 端口收敛到通用模型能力，保持 RFC 0003 行为兼容；
...
- 模型失败不产生部分 Revision，embedding 失败按 RFC 0003 降级；
