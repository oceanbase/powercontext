...
生成 Client 使用封闭枚举与 proposal 联合类型，最低支持级别为 **Dream contract 2**。本 PR 配套生成的 Python Client、CLI、MCP bridge 与 Dashboard 自动发送 `X-PowerContext-Dream-Contract: 2`；外部集成须升级相同契约并显式声明该级别。功能级别独立于包的发行版本，避免开发快照共用版本号造成误判。服务端在响应头声明最低级别；无声明或旧级别的 Client 仍可调用原有操作，capabilities 仅列出 `refine_experience`、`derive_skill`。读到新 Run、新 Candidate 或 `dream_review_approved` Profile 时返回 HTTP 426 `client_upgrade_required`，写新类型时在变更前拒绝。原类型响应移除新增的可选审计字段，绝不伪造 operation、mode 或空 Source window。部署验收覆盖新版 SDK 和无能力声明的旧客户端。
...
统一 Inbox 通过两个公共候选服务聚合展示，支持 `all/artifact/catalog_change` 类型筛选和 pending/approved/rejected 状态筛选。两种资源保持独立游标；全部视图分组展示，每组每页 12 条及独立下一页，不伪造跨资源时间顺序。详情展示目标差异、完整提案、精确证据、版本与决定记录，并明确 Prompt 发布、Skill 不安装和 Handoff 不激活的影响。批准、拒绝和修订均携带当前候选版本，通过现有权限检查；浏览器写入校验同源。此方案不创建新聚合表或 Inbox API，不创建 `family=tag`，不把新 Tag 审批混进要求 approved 必须有 result_artifact 的旧 DTO。首期不批量改名、不定义全局别名、不检查或修改外部系统自动化。涉及业务规则的标签需要业务方确认其影响，PowerContext 标签本身始终不授予权限。
...
1. 外部集成采用 Dream contract 2 的发布节奏；最低级别与旧 Client 升级行为已定义于协议章节。
...
4. 是否在有实际需求时增加跨资源时间排序；首期统一 Inbox 使用类型筛选与独立资源游标。
