...
Handoff Report 页面通过 `POST /v1/handoff-reports/projects/list` 获取当前 token 可访问的 Project，并以不可变的 `project_id` 作为“项目”下拉框的值。每个选项同时显示 Project title 和完整 `project_id`，避免同名 Project 无法区分。选择项目后，浏览器通过 `POST /v1/handoff-reports/get` 请求该 Project 的 canonical JSON，“交接工作”下拉框只列出属于当前项目的 Workstream。页面只负责格式化和展示 `summary`、`coverage`、Workstream 状态、Activity 数量、objective、current state、next action、known omissions 和 digest，不重新计算报告口径。
