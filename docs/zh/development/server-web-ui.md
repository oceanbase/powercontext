...
Handoff Report 页面通过 `POST /v1/handoff-reports/projects/list` 获取当前 token 可访问的 Project，并以不可变的 `project_id` 作为“项目”下拉框的值。每个选项同时显示 Project title 和完整 `project_id`，避免同名 Project 无法区分。选择项目后，浏览器通过 `POST /v1/handoff-reports/get` 请求该 Project 的 canonical JSON，并把该项目的 Workstream 展示为顶部横向切换区；切换项下方使用完整宽度显示当前精确交接快照，不再提供第二个下拉框。切换区发生溢出时会显示上一项、下一项和当前位置，并自动让所选工作项保持可见；工作项超过 8 个时，还会显示按名称或 scope 搜索的输入框。发送方和接手方工作台作为所选 Workstream 的二级操作展开，并在工作台标题处提供明确的关闭操作。活动周期控件与活动区域放在一起，因为它们只影响 Activity 数量。页面只负责格式化和展示 `summary`、`coverage`、Workstream 状态、Activity 数量、objective、current state、next action、known omissions 和 digest，不重新计算报告口径。
