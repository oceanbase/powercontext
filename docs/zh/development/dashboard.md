...
Dashboard 是 PowerContext 中查看已保存内容和用量的入口。这份文档供开发和评审人员判断页面该展示什么、如何组织阅读，以及一次改动是否保留了用户需要的行为。接口能力以 `openapi/powercontext.yaml` 和服务实现为依据。

## Dashboard 帮助用户完成什么

用户进入 Dashboard 时，可能想确认某项约定，查找解决过类似问题的经验，或继续一项中断的工作。页面需要让用户确定正在查看哪个范围，找到相关记录，并在需要时核对原始材料。

1.0 的核心是阅读已有内容。记忆、经验、技能和交接可以分别存在：只有记忆的范围也应当能够正常使用；没有内容的范围仍有明确的入口和页面结构。生成、审核和保存由现有工具与接口完成，阅读这些结果不应要求用户先配置生成能力或取得全局管理权限。
...
内容目录读取当前范围。用量可以选择当前范围或包含子范围，用户必须能辨认统计包含了哪些内容。例如，一个空子范围可以没有记忆，而父范围仍有记忆；这不构成数据缺失，也不能用父范围的内容填满子范围页面。
...
材料与记录之间的关系应当清楚。宽屏可以并排阅读，小屏使用抽屉，以便用户核对依据后回到正文。展开材料、切换材料和返回列表都有可见操作，键盘用户也能完成同样的过程。
...
Dashboard 使用当前用户的身份访问服务。能够读取一条记录，不意味着必须能列出全部范围；阅读单个范围也不应要求全局观察权限。页面需要遵守授权边界，并尽可能保留用户已获授权的阅读路径。
...
偏离框架默认行为需要具体原因。例如，页面片段替换可能提前移除尚未关闭的抽屉，此时需要通过组件公开接口完成清理。补充代码应解决这个副作用，并由相应的行为测试验证；通常的菜单定位和跨断点显隐仍交给框架。
...
| 权限变化、网络中断和服务恢复 | 已获授权的内容可读，错误明确，恢复路径有效 |
...
| 用量 | `POST /v1/stats` | 区分 `exact` 与 `subtree`，使用服务端周期与统计口径 |
