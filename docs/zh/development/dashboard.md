...
首页的交接、记忆、经验与技能、用量共用卡片外的区块标题和查看入口；有内容、空状态和读取失败时都保留标题。统计周期属于用量卡片内的内容。首页记忆使用 Tabler Card 和 List Group，双栏借助原生栅格与 Flex 工具类等高，卡片上下边缘与经验和技能区块对齐。xl 起，已有记忆条目均分卡片内的高度；窄屏按内容自然排列。记忆目录按分页容量预留条目行，摘要使用 Tabler 原生单行截断；当前页的所有条目完整排列，目录内不滚动。末页条目较少时，分页栏位置保持一致。记忆全文和经验／技能列表保留有界阅读区域。
