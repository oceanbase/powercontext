...
- 选择后立即切换页面，由 HTMX 的 `change` 事件提交表单；禁用脚本时提供提交按钮。选择器不修改默认 scope 或工具绑定。切换范围时清除记录身份和分页游标，详情页回到对应目录，并保留经验或技能类别。
- 选择项优先显示自身名称，括号中显示可读的祖先路径。选择器按 `parent_scope_id` 分组显示上级、子级和其他范围，不推断名称、目录或业务层级。
...
首页可以截短摘要并链接全文。详情不得通过拆句、改写或补写产生接口中没有的标题、结论或来源。标题沿用框架层级并自然换行，完整正文仍可见。
...
窄屏导航使用 Tabler Collapse，按需展开范围选择与菜单，桌面左栏随页面滚动停靠。页面以主标题开始，详情保留返回目录的链接。范围身份由选择器表达；加载状态浮于页面角落，不占用正文布局。

布局遵循 Tabler 的 [页面布局](https://docs.tabler.io/ui/layout/page-layouts) 和 [排版](https://docs.tabler.io/ui/base/typography) 规范，以项目固定的 1.4.0 样式为准。侧栏与主体使用原生栅格，在 lg 起占 3/9 列、xxl 起占 2/10 列；侧栏通过 `sticky-lg-top` 停靠，导航填满所属列，页面标题和正文共用 `container-xl`；内容用 `row`、`row-cards` 和 12 列栅格组织；`row-cards` 的间距随原生页面边距变化，避免窄屏负边距超出容器。首页两栏在 xl 起各占 6 列，记忆目录与正文为 4/8，详情正文与材料在 xxl 起为 7/5，用量摘要与图表在 xl 起为 4/8。首页简版用量在 lg 起为 4/8、xxl 起为 3/9。未达到相应断点时顺序堆叠，不能为容纳两栏缩小正文字号。

字体栈、字号、行高和字重使用 Tabler 1.4.0 默认值，不另设正文字体。默认根字号下，正文为 14 像素，`page-title-lg` 为 24 像素，分区标题为 20 像素，`card-title` 为 16 像素。条目使用 `fw-normal`，其余内容沿用组件字重。用量百分比使用原生 `display-3`，不能挤压标签、周期按钮或图表。周期选择使用原生小尺寸按钮组，需容纳窄屏英文标签。

记录引用在有值时直接显示为次要信息，不设置“记录标识”标题或折叠开关；相关引用为空时省略。引用列表随页面排列，不单独滚动。记忆正文和展开的用量明细采用有高度上限的滚动区域，保留完整内容和键盘访问。

材料面板使用 Tabler 的 `offcanvas-xxl`：大屏并排阅读，小屏使用原生抽屉和遮罩。跨断点显隐交给 Tabler。桌面材料栏高度为视口减去上下原生页面边距，并随正文滚动停靠；标题和操作区占自然高度，原文通过 Flex 占用剩余空间并独立滚动。材料标签过多时横向滚动；打开和切换材料不能增加整页高度。小屏保留原生 Offcanvas，标题、材料标签和正文保持可达。独立原文页同样在视口内阅读；错误区域允许滚动，不能裁掉恢复按钮。只有 HTMX 会提前移除正在关闭的节点时，才通过公开的 `hide`、关闭事件和 `dispose` 完成清理。图表在所属节点移除时销毁，避免重复监听和残留实例。 宽屏展开材料后提供“返回材料列表”，在本地清理材料内容并恢复链接焦点；返回不依赖网络，也不改变记录和页面阅读位置。小屏通过 Tabler 原生关闭按钮返回。

外层栅格使用 `align-content-start`，避免移动端短页面将视口剩余高度分配给导航行，造成正文起点下移。有内容、无内容和搜索无结果时，页面共用容器、标题、栅格与区块边界。记忆始终保留目录和阅读区；交接、经验与技能列表保留基础高度；用量保留摘要、图表位置和模型用量表。空图表沿用 Tabler 的 `chart`／`chart-lg` 尺寸，并在图表区域说明当前状态。首页摘要按标题、进展摘录和经验正文的行高预留空间，长内容自然增高。数值来自 API，空状态提示放在对应内容区域。

HTMX 替换页面后，css-scope-inline 会通过 MutationObserver 应用组件样式。图表用 Surreal 的 `run` 保存所属节点，再 `await tick()` 等待布局生效后实例化 ApexCharts；节点已被移除时取消创建。不能先按未生效的网格宽度绘制，再依赖尺寸监听修正。首页图表放在原生栅格列内，允许内容随容器缩小；不增加自定义 resize 监听。图表高度匹配原生 `chart`／`chart-lg`，字体继承页面，配色读取主题变量。日期轴使用 ApexCharts 原生 datetime 类型，按 UTC 日期显示并由图表处理标签密度。单日使用分类轴，只显示所选日期。

视觉保留白底、蓝色操作、细边界、左侧导航与正文分屏。品牌颜色通过主题变量设置，边界和页边距使用 Tabler 默认值；侧栏和主体的宽度由栅格列分配，页面只定义自身版式。检查 320 至 1920 像素的页面，包括 991/992、1199/1200 和 1399/1400 像素断点两侧。按浏览器缩放后的 CSS 视口检查，例如 1536 像素窗口在 200% 缩放下对应 768 像素布局。正文须留在页面和卡片边界内，不能只检查整页 scrollWidth；材料面板不能留下遮罩或滚动锁。

全局样式只保留白色页面背景和品牌主色变量。按钮、表单、卡片、圆角、焦点与过渡使用 Tabler 默认行为；不能用全局元素选择器重设组件样式。先用框架工具类组织布局，必要的等高、视口约束和正文换行规则放在所属组件的 css-scope-inline 样式内。Logo 使用 Tabler 工具类。

语言与外观集中在“显示设置”菜单，使用 Tabler Dropdown 的分组标题与分隔线，覆盖所有 Dashboard 页面、独立原文页与登录页。侧栏菜单放在 `.navbar-collapse` 外，避免 Tabler 将它作为子导航展开。通过 Dropdown 的公开 `popperConfig` 设置首选向上展开、启用 `applyStyles` 与 `preventOverflow.altAxis`，由框架处理短屏避让；导航栏默认静态定位无法保证此时的选项可达。打开菜单时按钮与正文保持原位。同一浏览器中的页面加载使用共用偏好。语言由 Jinja2 文案目录决定，`lang=zh|en` 写入仅限 Dashboard 的偏好 cookie；业务内容保留原文。切换链接保留 scope、周期和精确记录身份，并完整加载页面，确保根节点语言与 HTMX 片段一致。导航使用固定列数和可容纳两行文字的行高，显示设置使用单个入口；标题和周期区通过响应式列宽分配空间。用量的范围粒度选项在窄屏等分两列并预留换行高度。翻译可以自然换行，但不能改变导航顺序、按钮分组或主栏比例。材料面板直接显示完整正文。
...
完整 Logo 复用官网的 `website/assets/powercontext-color.png` 和 `powercontext-reverse.png`，通过 Tabler 的 `hide-theme-dark`、`hide-theme-light` 切换。favicon 用正方形 SVG 视窗显示原图左侧图形，不能包含文字或重新绘制品牌图形。记忆、经验和技能条目使用常规字重。列表摘要与阅读正文继承默认正文字号；经验、交接和技能详情使用相同的原生标题层级。记忆在 xl 断点以下顺序展示目录与正文，减少窄列内的频繁换行。用量表复用 Tabler `table-mobile-sm`，小屏按用途显示输入和输出，模型类型位于用途下方。每日明细保留可用键盘操作的原生横向滚动表格。记忆详情直接展示正文，不附加“阅读记忆”标题、版本栏或记录标识。箭头仅用于返回和新窗口等有方向含义的操作。导航不设置“开始使用”页，旧 `/dashboard/guide` 地址保留参数后重定向首页。

首页依次展示用量、记忆与经验及技能、交接；这些区块共用卡片外的区块标题和查看入口；有内容、空状态和读取失败时都保留标题。统计周期属于用量卡片内的内容。首页记忆使用 Tabler Card 和 List Group，双栏借助原生栅格与 Flex 工具类等高，卡片上下边缘与经验和技能区块对齐。xl 起，已有记忆条目均分卡片内的高度；窄屏按内容自然排列。记忆目录按分页容量预留条目行，摘要使用 Tabler 原生单行截断；当前页的所有条目完整排列，目录内不滚动。末页条目较少时，分页栏位置保持一致。经验和技能目录同样按分页容量分配行高，使用原生卡片和单行摘要，随页面滚动；完整内容通过详情页阅读。记忆正文在大屏与目录等高，长文在卡片内滚动。小屏按目录、正文顺序排列，阅读区高度受视口约束。
...
通过现有接口验证各类生成能力，再按精确引用读取保存后的记录。导入次日内容前检查前日召回与引用；用只读数据库查询核对 API 返回的条目、版本和每日用量。

检查后续要求能否取代早先约束。通过 API 修订后，当前召回应反映修正，历史引用仍应保留原文，过期引用写入应被拒绝。对比完整原文与预算内上下文保留的约束、下一步和引用；高精简比例不代表信息完整。实验输入和结果保存在仓库之外。

分别验证 scope 引用、目录归属和 subtree 统计，保留未关联的空子级用于首次使用检查，并保持已配置的默认范围。模拟日期可检验重启读取和每日归属，不能证明生产调度的长期可靠性或任意问题的召回质量。
