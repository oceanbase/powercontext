...
材料面板使用 Tabler 的 `offcanvas-xxl`：大屏并排阅读，小屏使用原生抽屉和遮罩。跨断点显隐交给 Tabler。桌面材料栏在视口内保持固定高度并随正文滚动停靠，原文在栏内独立滚动；打开和切换材料不能增加整页高度。小屏保留原生 Offcanvas，标题、材料标签和正文保持可达。独立原文页同样在视口内阅读；错误区域允许滚动，不能裁掉恢复按钮。只有 HTMX 会提前移除正在关闭的节点时，才通过公开的 `hide`、关闭事件和 `dispose` 完成清理。图表在所属节点移除时销毁，避免重复监听和残留实例。
