...
布局遵循 Tabler 的 [页面布局](https://docs.tabler.io/ui/layout/page-layouts) 和 [排版](https://docs.tabler.io/ui/base/typography) 规范，以项目固定的 1.4.0 样式为准。侧栏使用原生 `15rem` 宽度，正文和面包屑共用 `container-xl`；内容用 `row`、`g-4` 和 12 列栅格组织。首页两栏在 xl 起各占 6 列，记忆目录与正文为 4/8，详情正文与材料在 xxl 起为 7/5，用量摘要与图表在 xl 起为 4/8。首页简版用量在 lg 起为 4/8、xxl 起为 3/9。未达到相应断点时顺序堆叠，不能为容纳两栏缩小正文字号。

导航和正文以 16 像素为基准。字重沿用 Tabler 的常规 400、强调 500、标题 600；条目内容使用常规字重。页面主标题为 32 像素，小屏为 28 像素，分区标题采用原生 20 像素。用量百分比可放大，但不能挤压标签、周期按钮或图表。周期选择使用原生小尺寸按钮组，需容纳窄屏英文标签。

记录引用在有值时直接显示为次要信息，不设置“记录标识”标题或折叠开关；相关引用为空时省略。记忆正文、引用列表和展开的用量明细采用有高度上限的滚动区域，保留完整内容和键盘访问。

材料面板使用 Tabler 的 `offcanvas-xxl`：大屏并排阅读，小屏使用原生抽屉和遮罩。跨断点显隐交给 Tabler。桌面材料栏在视口内保持固定高度并随正文滚动停靠，原文在栏内独立滚动；打开和切换材料不能增加整页高度。小屏保留原生 Offcanvas，标题、材料标签和完整原文入口保持可达。独立原文页同样在视口内阅读；错误区域允许滚动，不能裁掉恢复按钮。只有 HTMX 会提前移除正在关闭的节点时，才通过公开的 `hide`、关闭事件和 `dispose` 完成清理。图表在所属节点移除时销毁，避免重复监听和残留实例。
...
视觉保留白底、蓝色操作、细边界、左侧导航与正文分屏。全局颜色、边界和页边距集中管理；侧栏宽度与正文偏移由 Tabler 管理，页面只定义自身版式。检查 320 至 1920 像素的页面，包括 991/992、1199/1200 和 1399/1400 像素断点两侧。按浏览器缩放后的 CSS 视口检查，例如 1536 像素窗口在 200% 缩放下对应 768 像素布局。正文须留在页面和卡片边界内，不能只检查整页 scrollWidth；材料面板不能留下遮罩或滚动锁。
...
语言与外观使用共享的 Tabler Dropdown，覆盖所有 Dashboard 页面、独立原文页与登录页。同一浏览器中的页面加载使用共用偏好。语言由 Jinja2 文案目录决定，`lang=zh|en` 写入仅限 Dashboard 的偏好 cookie；业务内容保留原文。切换链接保留 scope、周期和精确记录身份，并完整加载页面，确保根节点语言与 HTMX 片段一致。
