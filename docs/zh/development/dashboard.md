...
首页记忆使用 Tabler Card 和 List Group，双栏借助原生栅格与 Flex 工具类等高，卡片上下边缘与经验和技能区块对齐。xl 起，已有记忆条目均分卡片内的高度；窄屏按内容自然排列。记忆目录、正文和经验／技能列表采用稳定高度的独立滚动区域；分页栏位于区域底部，条目数量与正文长度不改变其位置。
