...
PowerContext provides official plugins and installation guides for Codex, Claude Code, DeepSeek Harness, and Pi. All
integrations use the same scoped data and history-preserving contracts through PowerContext Server; the host
integrations do not start or embed the Server.
...
You need macOS or Linux, Python 3.11 or newer, [`uv`](https://docs.astral.sh/uv/), and at least one supported host.
...
powercontext setup pi --source oceanbase/powercontext --ref v0.0.2
```
...
```bash
powercontext doctor
powercontext doctor codex  # or: claude-code / dsh / pi
```
