...
import type { Message, MessageResponse, Memory } from '@/types';
...
export class MessageHandler {
  private apiClient: PowerMemClient | null = null;
  ...
  async handle(message: Message, sender: chrome.runtime.MessageSender): Promise<MessageResponse> {
    ...
    try {
      ...
      ErrorHandler.log(error as Error, { message, sender });
   ...
   * @returns 保存结果
   */
  private async handleSaveMemory(payload: any): Promise<MessageResponse<Memory>> {
   ...
   * @returns 搜索结果
   */
  private async handleSearchMemories(payload: any): Promise<MessageResponse<Memory[]>> {
   ...
   * @returns 记忆列表
   */
  private async handleGetMemories(payload: any): Promise<MessageResponse> {
    const settings = await StorageService.getSettings();
    const client = await this.getApiClient();
   ...
   * @returns 连接测试结果
   */
  private async handleTestConnection(): Promise<MessageResponse> {
    try {
      const client = await this.getApiClient();
   ...
   * @returns API 客户端实例
   */
  private async getApiClient(): Promise<PowerMemClient> {
    if (!this.apiClient) {
      const settings = await StorageService.getSettings();
      ...
      this.apiClient = new PowerMemClient(
        settings.server.url,
   ...
   */
  private validatePayload(payload: any, required: string[]): void {
    if (!payload) {
