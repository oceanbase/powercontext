...
import type { Memory, MemoryListResponse, MemorySearchResponse } from '@/types';
...
export class PowerMemClient {
  ...
  async createMemory(memory: Partial<Memory>): Promise<Memory> {
    try {
      const response = await this.client.post<Memory>('/memories', memory);
  ...
  async searchMemories(params: {
    ...
    try {
      const response = await this.client.post<MemorySearchResponse>('/memories/search', {
        query: params.query,
  ...
  async getMemories(params: {
    ...
    try {
      ...
      const response = await this.client.get<MemoryListResponse>(
  ...
  async updateMemory(memoryId: string, updates: Partial<Memory>): Promise<Memory> {
    try {
      const response = await this.client.put<Memory>(`/memories/${memoryId}`, updates);
      return response.data;
    } catch (error) {
      throw error;
    }
