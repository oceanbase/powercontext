...
export class MemoryService {
   ...
   * @returns 创建的记忆
   */
  static async create(memory: Partial<Memory>): Promise<Memory> {
    try {
   ...
   * @returns 更新后的记忆
   */
  static async update(memoryId: string, updates: Partial<Memory>): Promise<Memory> {
    try {
      const response = await sendMessage({
        type: 'UPDATE_MEMORY',
        payload: {
   ...
   */
  static async delete(memoryId: string, skipConfirm: boolean = false): Promise<void> {
    try {
      // 显示确认对话框
      if (!skipConfirm) {
   ...
   * @param skipConfirm - 跳过确认对话框
   */
  static async batchDelete(memoryIds: string[], skipConfirm: boolean = false): Promise<void> {
    try {
      // 显示确认对话框
      if (!skipConfirm) {
   ...
   * @returns 记忆对象
   */
  static async getById(memoryId: string): Promise<Memory> {
    try {
      const response = await sendMessage({
        type: 'GET_MEMORY',
        payload: { memoryId },
   ...
   * @returns 是否确认删除
   */
  private static async confirmDelete(memoryId: string): Promise<boolean> {
    // 检查是否设置了"不再提示"
   ...
   * @returns 是否确认删除
   */
  private static async confirmBatchDelete(count: number): Promise<boolean> {
    // 检查是否设置了"不再提示"
    const result = await chrome.storage.local.get('skipDeleteConfirm');
    if (result.skipDeleteConfirm) {
      return true;
