...
import { PREDEFINED_AGENTS } from '@/types/agent';
/**
 * Agent 存储键
 */
...
export class AgentManager {
     ...
     */
    static async switchAgent(agentId) {
        try {
            // 验证 Agent 是否存在
            const allAgents = await this.getAllAgents();
    ...
    static async deleteAgent(agentId) {
        try {
            ...
            const result = await chrome.storage.local.get(STORAGE_KEY);
            const customAgents = result[STORAGE_KEY] || [];
            const filteredAgents = customAgents.filter((a) => a.id !== agentId);
     ...
     * @returns 记忆数量
     */
    static async getAgentMemoryCount(agentId) {
        try {
            // 这里需要通过消息传递调用后端 API
            // 暂时返回 0，在后端集成时实现
