...
export class RecommendationEngine {
  ...
  static getRecommendations(
    ...
    // 计算每个记忆的相关性分数
    const results: RecommendationResult[] = memories.map(memory => {
   ...
   * @returns 分数 (0-1)
   */
  private static calculateDomainScore(
    memoryDomain: string,
   ...
   * @returns 相似度 (0-1)
   */
  private static calculateUrlSimilarity(
    memoryUrl: string,
    currentUrl: string
  ): number {
   ...
   * @returns 衰减系数 (0-1)
   */
  private static calculateTimeDecay(createdAt: string): number {
    if (!createdAt) {
      return 0.5; // 默认值
    }
