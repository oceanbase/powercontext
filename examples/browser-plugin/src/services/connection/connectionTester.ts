...
export class ConnectionTester {
   ...
   * @returns 是否健康
   */
  private static async testHealthCheck(client: PowerMemClient): Promise<boolean> {
    try {
   ...
   * @returns 是否认证成功
   */
  private static async testAuthentication(client: PowerMemClient): Promise<boolean> {
    try {
      // 使用临时用户 ID 测试认证
   ...
   * @returns 错误详情
   */
  private static parseError(error: any): ConnectionTestResult['errorDetails'] {
    // 网络错误
