    ...
    def __init__(
        ...
        **kwargs,
        ...
        if embedding_dims is None:
            embedding_dims = dims
        super().__init__(
            embedding_dims=embedding_dims, model=model, api_key=api_key, **kwargs
        )
        self.dims = dims
        ...
        **kwargs,
        ...
        if messages:
            ...
            if response_format and response_format.get("type") == "json_object":
                ...
                return json.dumps(
                    {"facts": facts if facts else ["General conversation"]}
                )
...
def test_step1_custom_llm_provider() -> None:
    ...
    try:
        ...
        try:
            result = memory.add(
                "Test memory with custom LLM", user_id="test_user_step1", infer=False
            )
...
def test_step2_custom_embedder_provider() -> None:
    ...
    try:
        ...
        try:
            result = memory.add(
                "Test memory with custom embedder",
...
class CustomVectorStore(VectorStoreBase):
    ...
    def insert(self, vectors, payloads=None, ids=None):
        ...
        if ids is None:
            ids = [
                f"mem_{len(self._vectors[col_name]) + i}" for i in range(len(vectors))
            ]
    ...
    def delete(self, vector_id):
        ...
        if col_name in self._storage and vector_id in self._storage[col_name]:
            ...
            self._vectors[col_name] = [
                v for v in self._vectors[col_name] if v["id"] != vector_id
            ]
            return True
        return False
    ...
    def col_info(self):
        ...
        if col_name in self._storage:
            return {
                ...
                "vector_size": len(self._vectors[col_name][0]["vector"])
                if self._vectors[col_name]
                else 0,
            }
        return None
...
def test_step3_custom_vector_store() -> None:
    ...
    try:
        ...
        try:
            result = memory.add(
                "Test memory with custom vector store",
                user_id="test_user_step3",
                infer=False,
            )
            print("✓ Memory added successfully")
            print(f"  Result: {result.get('results', [])}")
...
def test_step4_langchain_integration() -> None:
    ...
    try:
        ...
        print(
            "  Install with: pip install langchain>=1.1.0 langchain-core>=1.1.0 langchain-openai>=1.1.0 langchain-community>=0.4.1"
        )
        return
    ...
    try:
        ...
        langchain_memory = PowermemLangChainMemory(
            powermem_instance=powermem_instance, user_id="langchain_test_user"
        )
    ...
    try:
        ...
        try:
            ...
            print(
                "⚠ Some LangChain components not available, skipping full integration test"
            )
        ...
        if FULL_LANGCHAIN_AVAILABLE:
            ...
            langchain_memory_full = PowermemLangChainMemory(
                powermem_instance=powermem_full, user_id="langchain_full_test_user"
            )
            print("  ✓ PowermemLangChainMemory instance created")
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        "You are a helpful assistant. Use the following context to provide personalized responses.",
                    ),
            ...
            chain = RunnableLambda(format_messages) | prompt | mock_llm
        ...
        if FASTAPI_AVAILABLE:
            ...
            if "custom" not in LLMFactory.provider_to_class:
                LLMFactory.register_provider(
                    "custom", f"{__name__}.CustomLLM", CustomLLMConfig
                )
                    ...
                    try:
                        ...
                        if isinstance(result, dict) and "results" in result:
                            ...
                            if added_count > 0:
                                ...
                                print(
                                    f"  ⚠ Memory operation completed but no results returned"
                                )
                        else:
                        ...
                        search_req = SearchRequest(
                            query="Python", user_id=test_user_id, limit=5
                        )
                        ...
                        if isinstance(result, dict) and "results" in result:
                            ...
                            if remaining_count == 0:
                                print(
                                    f"  ✓ Verification: All memories deleted (0 remaining)"
                                )
                            else:
                                print(
                                    f"  ⚠ Verification: {remaining_count} memories still remain"
                                )
                    ...
                    try:
                                ...
                                for pid in pids:
                                    try:
                                        ...
                                        try:
                                            ...
                                            if (
                                                "python" in proc_name
                                                or "uvicorn" in proc_name
                                            ):
                                                print(
                                                    f"  Found server process (PID {pid}) on port {port}"
                                                )
                                                ...
                                                try:
                                                    ...
                                                    print(
                                                        f"  Process still running, sending SIGKILL..."
                                                    )
                                                    os.kill(pid_int, signal.SIGKILL)
                                                    print(
                                                        f"  ✓ Force killed process {pid}"
                                                    )
                                                except ProcessLookupError:
                                                    print(
                                                        f"  ✓ Process {pid} stopped gracefully"
                                                    )
                                        except (
                                            ValueError,
                        ...
                        try:
                            ...
                            if f":{port}" in result.stdout:
                                ...
                                print(
                                    f"  ⚠ Port {port} appears to be in use, but couldn't determine PID"
                                )
                        except (FileNotFoundError, subprocess.TimeoutExpired):
                            pass
    ...
    try:
        ...
        class CustomIntelligencePlugin(IntelligentMemoryPlugin):
            ...
            def calculate_retention(
                self, memory: Dict[str, Any], time_since_creation: float
            ) -> float:
            ...
            def on_add(
                ...
                return {"importance": importance, "custom_field": "custom_value"}
        ...
        # Test custom plugin
        plugin_config = {"enabled": True, "decay_rate": 0.1}
...
def test_complete_example() -> None:
    ...
    try:
        ...
        try:
            ...
            if (
                "Unsupported" in stderr_content
                or "validation error" in stderr_content.lower()
            ):
                print(
                    "  ℹ Note: Pydantic validation warnings are expected for custom providers"
                )
            ...
            try:
                result = memory.add(
                    "Test memory from complete example", user_id="test_user"
                )
                print(f"✓ Memory added successfully: {result}")
            except Exception as e:
                print(
                    f"⚠ Could not add memory (expected if API keys missing): {str(e)[:100]}"
                )
            ...
            try:
                ...
                print(
                    f"⚠ Could not search (expected if API keys missing): {str(e)[:100]}"
                )
...
def main() -> None:
    ...
    print("\n✓ Registration Status:")
    print(
        f"  - Custom LLM Provider: {'✓' if 'custom' in LLMFactory.provider_to_class else '✗'}"
    )
    print(
        f"  - Custom Embedder Provider: {'✓' if BaseEmbedderConfig.has_provider('custom') else '✗'}"
    )
    print(
        f"  - Custom Vector Store Provider: {'✓' if 'custom' in VectorStoreFactory.provider_to_class else '✗'}"
    )
