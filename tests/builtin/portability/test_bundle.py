    ...
    async def scenario() -> None:
        ...
        async with SQLiteProfile.open(SQLiteConfig(), tables=BUILTIN_TABLES) as target:
            ...
            async with target.database.transaction() as connection:
                assert (await connection.execute(select(SCOPES_TABLE))).all() == []
...
def test_bundle_preserves_registered_family_dependencies_and_tags(tmp_path: Path) -> None:
    async def scenario() -> None:
        archive = tmp_path / "families.pcb"
        now = datetime(2026, 1, 2, 3, 4, 5, tzinfo=UTC)
        package_path = tmp_path / "portable"
        package_path.mkdir()
        (package_path / "SKILL.md").write_text(
            "---\nname: portable\ndescription: portable skill\n---\nRead guide.md.\n"
        )
        (package_path / "guide.md").write_text("Verify the restored package.\n")
        package = capture_skill_directory(package_path)
        skill_content = package.as_skill_content().model_dump_json().encode()
        async with SQLiteProfile.open(SQLiteConfig(), tables=BUILTIN_TABLES) as source:
            async with source.database.transaction() as connection:
                await _insert_scope(connection)
                await connection.execute(
                ...
                await connection.execute(
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            "artifact_id": "topic",
                            "revision": 1,
                            "content": b'{"title":"Topic","summary":"Summary","detail":"Detail"}',
                        },
                        {
                            "scope_id": "project:one",
                            "family": "prompt",
                ...
                await connection.execute(
                    insert(ARTIFACT_HEADS_TABLE),
                    [
                        {"scope_id": "project:one", "family": "skill", "artifact_id": "portable", "revision": 1},
                        {"scope_id": "project:one", "family": "topic-memory", "artifact_id": "topic", "revision": 1},
                        {"scope_id": "project:one", "family": "prompt", "artifact_id": "prompt", "revision": 1},
                    ],
                )
                await connection.execute(
                ...
                await connection.execute(
                    insert(ARTIFACT_TAGS_TABLE).values(
                        ...
                        target_id="portable",
                        tag_key_hash=hashlib.sha256(b"team").digest(),
                        tag_key="team",
                        tag="platform",
                        assigned_at=now,
                    )
        ...
        async with SQLiteProfile.open(SQLiteConfig(), tables=BUILTIN_TABLES) as target:
            service = PortableBundleService(
                target.database,
                supported_artifact_families=("skill", "topic-memory", "prompt", "profile"),
            )
            validation = await service.validate(archive)
            assert validation.compatible is True
            assert validation.required_artifact_families == ("profile", "prompt", "skill", "topic-memory")
            await service.restore(archive)
            async with target.database.transaction() as connection:
                assert await connection.scalar(select(SKILL_PACKAGES_TABLE.c.archive_bytes)) == package.archive_bytes
                assert await connection.scalar(select(ARTIFACT_TAGS_TABLE.c.tag)) == "platform"
                assert await connection.scalar(select(PROFILE_POLICIES_TABLE.c.activation_mode)) == "automatic"
                assert (
                    await connection.scalar(select(TOPIC_MEMORY_REVISION_PUBLICATIONS_TABLE.c.artifact_id)) == "topic"
                )
    ...
    async def scenario() -> None:
        ...
        async with SQLiteProfile.open(SQLiteConfig(), tables=BUILTIN_TABLES) as source:
            async with source.database.transaction() as connection:
                ...
                await connection.execute(
                    insert(ARTIFACTS_TABLE).values(
                        scope_id="project:one", family="skill", artifact_id="broken", revision=1, content=content
                    )
                )
                await connection.execute(
                    insert(ARTIFACT_HEADS_TABLE).values(
...
def test_sqlite_export_uses_one_snapshot_during_concurrent_capture(tmp_path: Path) -> None:
    async def scenario() -> None:
        database_path = tmp_path / "concurrent.db"
        archive = tmp_path / "snapshot.pcb"
        async with SQLiteProfile.open(
            SQLiteConfig(url=f"sqlite+aiosqlite:///{database_path}"), tables=BUILTIN_TABLES
        ) as source:
            async with source.database.transaction() as connection:
                await _insert_scope(connection)
                await connection.execute(insert(SOURCE_JOURNAL_HEADS_TABLE).values(scope_id="project:one", position=0))
            committed = False

            def capture_during_export(progress):
                nonlocal committed
                if progress.phase != "export" or committed:
                    return
                with sqlite3.connect(database_path) as writer:
                    writer.execute(
                        "UPDATE pc_source_journal_heads SET position = 1 WHERE scope_id = ?", ("project:one",)
                    )
                    writer.execute(
                        "INSERT INTO pc_sources (scope_id, source_type, source_id, payload, journal_position) "
                        "VALUES (?, ?, ?, ?, ?)",
                        ("project:one", "content", "concurrent", b"{}", 1),
                    )
                committed = True

            await PortableBundleService(source.database).export(
                ["project:one"], archive, authorize=_authorize_export, progress=capture_during_export
            )
            assert committed
        async with SQLiteProfile.open(SQLiteConfig(), tables=BUILTIN_TABLES) as target:
            await PortableBundleService(target.database).restore(archive)
            async with target.database.transaction() as connection:
                assert await connection.scalar(select(SOURCE_JOURNAL_HEADS_TABLE.c.position)) == 0
