...
class _RecordingVectorIndex(NoMemoryIndex):
    ...
    async def delete(
        ...
        self.projections = tuple(p for p in self.projections if p.entry_version.entry_id not in entry_ids)
    ...
    async def upsert(
        ...
        by_entry = {p.entry_version.entry_id: p for p in self.projections}
        by_entry.update((p.entry_version.entry_id, p) for p in projections)
        self.projections = tuple(by_entry.values())
