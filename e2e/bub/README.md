...
Local workloads may add `batch`. Two or more compatible selected tasks in the same batch share one run-local Harbor
task and container; their steps come from the source Harbor tasks, and their scopes, evidence, and evaluation remain
independent. Selecting one task uses the normal single-task path.
...
| --- | --- | --- | --- |
| `locomo-*` (four manifests) | one local Harbor task each; `batch: locomo` | `acceptance`, `sample` | Pinned LoCoMo-derived cases |
